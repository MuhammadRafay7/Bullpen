...
const JWT_SECRET_ENV_KEY = "BULLPEN_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Bullpen CLI commands",
...
export function resolveBullpenEnvFile(configPath?: string): string {
...
export function loadBullpenEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeBullpenEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readBullpenEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeBullpenEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeBullpenEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readBullpenEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeBullpenEnvEntries(next, filePath);
  return next;
}
