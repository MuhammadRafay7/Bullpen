...
export const DEFAULT_WORKTREE_HOME = "~/.bullpen-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".bullpen");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: BullpenConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "bullpen",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    BULLPEN_HOME: paths.homeDir,
    BULLPEN_INSTANCE_ID: paths.instanceId,
    BULLPEN_CONFIG: paths.configPath,
    BULLPEN_CONTEXT: paths.contextPath,
    BULLPEN_IN_WORKTREE: "true",
    BULLPEN_DB_BACKUP_ENABLED: "false",
    ...(branding?.name ? { BULLPEN_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { BULLPEN_WORKTREE_COLOR: branding.color } : {}),
  };
}
