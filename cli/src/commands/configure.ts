...
function defaultConfig(): BullpenConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" bullpen configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `bullpen onboard` first.");
    p.outro("");
    process.exitCode = 1;
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because BULLPEN_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
