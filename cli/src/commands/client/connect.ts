...
import type { Agent, Company } from "@bullpen/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@bullpen/shared";
import { loginBoardCli } from "../../client/board-auth.js";
import { BullpenApiClient } from "../../client/http.js";
...
export function registerConnectCommand(program: Command): void {
  addCommonClientOptions(
    program
      ...
      .option("--api-key-env-var-name <name>", "Env var name to store in the profile", "BULLPEN_API_KEY")
...
async function connectWizard(opts: ConnectOptions) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("`bullpen connect` is interactive. For scripts, pass --api-base/--api-key or use context set/token commands.");
  }

  p.intro(pc.bgCyan(pc.black(" bullpen connect ")));
  ...
  const boardLogin = await loginBoardCli({
    ...
    command: "bullpen connect",
  });
  const boardApi = new BullpenApiClient({ apiBase, apiKey: boardLogin.token });
  ...
  const apiKeyEnvVarName = opts.apiKeyEnvVarName?.trim() || "BULLPEN_API_KEY";
...
async function verifyHealth(apiBase: string): Promise<void> {
  const api = new BullpenApiClient({ apiBase });
...
function buildExports(input: {
  ...
  return [
    `export BULLPEN_API_URL='${escaped(input.apiBase)}'`,
    input.companyId ? `export BULLPEN_COMPANY_ID='${escaped(input.companyId)}'` : null,
    input.agentId ? `export BULLPEN_AGENT_ID='${escaped(input.agentId)}'` : null,
