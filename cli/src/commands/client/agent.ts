...
function buildAgentEnvExports(input: {
  ...
  return [
    ...
    `export BULLPEN_COMPANY_ID='${escaped(input.companyId)}'`,
    `export BULLPEN_AGENT_ID='${escaped(input.agentId)}'`,
    `export BULLPEN_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Bullpen skills for Codex/Claude, and print shell exports",
      )
      .argument("<agentRef>", "Agent ID or shortname/url-key")
      .requiredOption("-C, --company-id <id>", "Company ID")
      ...
      .option(
        "--no-install-skills",
        "Skip installing Bullpen skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveBullpenSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Bullpen skills directory. Expected ./skills in the repo checkout.",
              );
            }
