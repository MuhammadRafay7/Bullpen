...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@bullpen/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://bullpen:bullpen@127.0.0.1:${port}/bullpen`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" bullpen db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveBullpenInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "bullpen";
