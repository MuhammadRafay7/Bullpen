...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" bullpen run ")));
  ...
  if (!configExists(configPath)) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
      ...
      p.log.message(`Run ${pc.cyan("bullpen onboard")} once, then retry ${pc.cyan("bullpen run")}.`);
      process.exit(1);
    }
  ...
  p.log.step("Starting Bullpen server...");
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  if (process.env.BULLPEN_UI_DEV_MIDDLEWARE !== undefined) return;
  const normalized = entrypoint.replaceAll("\\", "/");
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@bullpen/server/src/index.ts")) {
    process.env.BULLPEN_UI_DEV_MIDDLEWARE = "true";
  }
}
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the Bullpen dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @bullpen/server package
  try {
    const mod = await import("@bullpen/server");
    return await startServerFromModule(mod, "@bullpen/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@bullpen/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Bullpen server entrypoint.\n` +
          `Tried: ${devEntry}, @bullpen/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `Bullpen server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}

function shouldGenerateBootstrapInviteAfterStart(config: BullpenConfig): boolean {
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`Bullpen server entrypoint did not export startServer(): ${label}`);
  }
