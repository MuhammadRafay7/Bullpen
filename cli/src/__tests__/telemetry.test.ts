...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.BULLPEN_HOME = path.join(root, "home");
    process.env.BULLPEN_INSTANCE_ID = "telemetry-test";
