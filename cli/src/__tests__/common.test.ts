...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.BULLPEN_AUTH_STORE;
    delete process.env.BULLPEN_SERVER_PORT;
  });
  ...
  it("resolves api base by explicit, env, profile, then config/default precedence", () => {
    ...
    process.env.BULLPEN_API_URL = "http://env:3/";
  ...
  it("prefers explicit and env tokens over profile env and stored board auth", () => {
    ...
    process.env.BULLPEN_AUTH_STORE = authStorePath;
    ...
    process.env.BULLPEN_API_KEY = "env-token";
