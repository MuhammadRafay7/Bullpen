...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".bullpen", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-onboard-fresh-"));
  return path.join(root, ".bullpen", "config.json");
}
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.BULLPEN_DB_BACKUP_DIR;
    delete process.env.BULLPEN_DB_BACKUP_ENABLED;
    delete process.env.BULLPEN_DB_BACKUP_INTERVAL_MINUTES;
    delete process.env.BULLPEN_DB_BACKUP_RETENTION_DAYS;
    delete process.env.BULLPEN_STORAGE_PROVIDER;
    delete process.env.BULLPEN_STORAGE_LOCAL_DIR;
    delete process.env.BULLPEN_SECRETS_PROVIDER;
    delete process.env.BULLPEN_SECRETS_STRICT_MODE;
    delete process.env.BULLPEN_HOME;
    delete process.env.BULLPEN_CONFIG;
    delete process.env.BULLPEN_INSTANCE_ID;
    delete process.env.BULLPEN_BIND;
    delete process.env.BULLPEN_BIND_HOST;
    delete process.env.BULLPEN_TAILNET_BIND_HOST;
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.BULLPEN_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as BullpenConfig;
  ...
  it("creates instance-root config and data paths for a fresh BULLPEN_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "bullpen-onboard-cwd-"));
    process.chdir(cwd);
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.BULLPEN_TAILNET_BIND_HOST = "100.64.0.8";
