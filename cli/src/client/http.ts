...
export class BullpenApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-bullpen-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Bullpen API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Bullpen server is not running, the configured URL is wrong, or the request is being blocked before it reaches Bullpen.",
    "",
    "Try:",
    "- Start Bullpen with `pnpm dev` or `pnpm bullpen run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Bullpen is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`BULLPEN_API_URL\`.`,
  );
  return lines.join("\n");
}
