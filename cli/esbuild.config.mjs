/**
 * esbuild configuration for building the bullpen CLI for npm.
 *
 * Bundles all workspace packages (@bullpen/*) into a single file.
...
// to npm and resolved at runtime (e.g. @bullpen/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@bullpen/") && !bundledCliNpmDependencies.has(name)) {
      externals.add(name);
    }
  }
