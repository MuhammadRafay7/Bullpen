...
export function resolveWorktreeEnvFilePath(rootDir: string): string {
  return path.resolve(rootDir, ".bullpen", ".env");
}
...
function resolveDefaultWorktreeHome(env: NodeJS.ProcessEnv): string {
  return path.resolve(expandHomePrefix(env.BULLPEN_WORKTREES_DIR?.trim() || "~/.bullpen-worktrees"));
}
...
function repairStaleMigratedWorktreeEnvEntries(
  ...
  const localConfigPath = path.resolve(rootDir, ".bullpen", "config.json");
  const configuredPath = entries.BULLPEN_CONFIG?.trim();
  ...
  return {
    ...
    BULLPEN_CONFIG: localConfigPath,
    BULLPEN_CONTEXT: path.resolve(homeDir, "context.json"),
  };
}
