...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "bullpen", LOGNAME: "bullpen", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "bullpen" }),
      },
    );

    expect(tokens).toEqual(["bullpen", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "bullpen" }, {
        userInfo: () => ({ username: "bullpen" }),
      });

      expect(tokens).toEqual(["bullpen", "custom-token"]);
    } finally {
