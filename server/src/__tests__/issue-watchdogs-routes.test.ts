...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("bullpen-issue-watchdogs-routes-");
  ...
  async function seedCompany(name = "Bullpen") {
  ...
  it("routes watchdog-discovered product bugs outside the watched source tree with evidence links", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        description: "Watchdog found a Bullpen follow-up routing bug.",
