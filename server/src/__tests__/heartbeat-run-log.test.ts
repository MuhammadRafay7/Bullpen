...
describe("compactRunLogChunk", () => {
  ...
  it("truncates oversized chunks after sanitizing them", () => {
    ...
    expect(compacted).toContain("[bullpen truncated run log chunk:");
    expect(compacted.endsWith("tail")).toBe(true);
  });

  it("redacts Bullpen credential shapes before persisting run-log chunks", () => {
    const chunk = [
      "Authorization: Bearer live-bearer-token-value",
      `export BULLPEN_API_KEY='bullpen-shell-secret'`,
      `auth {"refresh_token":"refresh-token-fixture-secret"}`,
      `payload {"BULLPEN_API_KEY":"bullpen-json-secret"}`,
      "--bullpen-api-key=bullpen-flag-secret",
    ].join("\n");
    ...
    expect(compacted).not.toContain("bullpen-shell-secret");
    expect(compacted).not.toContain("refresh-token-fixture-secret");
    expect(compacted).not.toContain("bullpen-json-secret");
    expect(compacted).not.toContain("bullpen-flag-secret");
  });
});
