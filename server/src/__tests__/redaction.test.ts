...
describe("redaction", () => {
  ...
  it("redacts common secret shapes from unstructured text", () => {
    ...
    const input = [
      ...
      `bullpen {"BULLPEN_API_KEY":"bullpen-json-secret"}`,
    ...
    expect(result).not.toContain("bullpen-json-secret");
    expect(result).not.toContain("escaped-json-secret");
    expect(result).not.toContain("bullpen-shell-secret");
  ...
  it("redacts inline secrets from command metadata without hiding safe command text", () => {
    const input = {
      ...
      env: {
        BULLPEN_RESOLVED_COMMAND: "env OPENAI_API_KEY=sk-live-example custom-acp --token ghp_example_secret",
        SAFE_VALUE: "visible",
      },
    };
