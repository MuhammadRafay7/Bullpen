...
import { agents, companies, createDb } from "@bullpen/db";
import {
...
describeEmbeddedPostgres("teams catalog install with no caller adapter overrides", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bullpen-teams-catalog-no-overrides-"));
    process.env.BULLPEN_HOME = tempHome;
    tempDb = await startEmbeddedPostgresTestDatabase("bullpen-teams-catalog-no-overrides-");
