...
import { execute, testEnvironment } from "@bullpen/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@bullpen/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("bullpen:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("BULLPEN_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("BULLPEN_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Bullpen Wake Payload");
      ...
      expect(payload?.bullpen).toBeUndefined();
