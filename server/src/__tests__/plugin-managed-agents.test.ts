...
function manifest(): BullpenPluginManifestV1 {
  return {
    id: "bullpen.managed-agents-test",
    apiVersion: 1,
    version: "0.1.0",
...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("bullpen-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: BullpenPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@bullpen/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    ...
    const previousInstance = process.env.BULLPEN_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "bullpen-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "bullpen-managed-agent-wiki-")));
    ...
    try {
      ...
      if (previousInstance === undefined) delete process.env.BULLPEN_INSTANCE_ID;
      else process.env.BULLPEN_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "bullpen.managed-agents-test",
