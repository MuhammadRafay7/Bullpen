...
describe("normalizeIssueAttachmentMaxBytes", () => {
  it("keeps the process-level attachment cap as the final cap", async () => {
    const previous = process.env.BULLPEN_ATTACHMENT_MAX_BYTES;
    process.env.BULLPEN_ATTACHMENT_MAX_BYTES = "5";
    vi.resetModules();
    try {
      ...
      if (previous === undefined) {
        delete process.env.BULLPEN_ATTACHMENT_MAX_BYTES;
      } else {
        process.env.BULLPEN_ATTACHMENT_MAX_BYTES = previous;
      }
      vi.resetModules();
    }
  });
...
describe("issue attachment routes", () => {
  ...
  it("canonicalizes bullpen artifact metadata before creating a work product", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
  ...
  it("rejects bullpen artifact metadata that references another issue's attachment", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
  ...
  it("canonicalizes bullpen artifact metadata on work product updates", async () => {
    const storage = createStorageService();
    const issue = {
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-1",
