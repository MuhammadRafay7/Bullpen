...
describe("resolveEnvironmentExecutionTarget", () => {
  ...
  it("keeps sandbox targets on bridge mode even when lease metadata includes a Bullpen API URL", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      driver: "sandbox",
      config: {
        provider: "fake-plugin",
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        bullpenApiUrl: "https://bullpen.example.test",
      },
      lease: null,
      environmentRuntime: null,
    });
    ...
    expect(target).not.toHaveProperty("bullpenApiUrl");
    expect(target).not.toHaveProperty("bullpenTransport");
  });
  ...
  it("keeps sandbox targets on callback bridge execution even when lease metadata advertises SSH access", async () => {
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        remoteCwd: "/home/sandbox/bullpen-workspace",
        sshAccess: {
          type: "ssh",
