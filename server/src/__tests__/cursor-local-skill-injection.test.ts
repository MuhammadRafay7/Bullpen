...
import { ensureCursorSkillsInjected } from "@bullpen/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Bullpen skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("bullpen-cursor-skills-src-");
    const skillsHome = await makeTempDir("bullpen-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "bullpen");
    await createSkillDir(skillsDir, "bullpen-create-agent");
    ...
    const injectedA = path.join(skillsHome, "bullpen");
    const injectedB = path.join(skillsHome, "bullpen-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "bullpen")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "bullpen-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "bullpen"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "bullpen-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("bullpen-cursor-preserve-src-");
    const skillsHome = await makeTempDir("bullpen-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "bullpen");
    ...
    expect((await fs.lstat(path.join(skillsHome, "bullpen-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("bullpen-cursor-fail-src-");
    const skillsHome = await makeTempDir("bullpen-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
