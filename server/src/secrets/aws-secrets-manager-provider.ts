...
import type { DeploymentMode, SecretProviderConfigDiscoveryPreviewResult } from "@bullpen/shared";
...
const DEFAULT_PREFIX = "bullpen";
const DEFAULT_OWNER_TAG = "bullpen";
const DEFAULT_VERSION_STAGE = "AWSCURRENT";
const BULLPEN_PENDING_VERSION_STAGE = "BULLPEN_PENDING";
...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Bullpen server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Bullpen company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function readProviderVaultConfig(input: SecretProviderVaultRuntimeConfig): AwsSecretsManagerConfig {
  ...
  const recoveryWindowRaw = process.env.BULLPEN_SECRETS_AWS_DELETE_RECOVERY_DAYS?.trim();
  ...
  if (!Number.isFinite(recoveryWindow) || recoveryWindow < 7 || recoveryWindow > 30) {
    throw unprocessable(
      "BULLPEN_SECRETS_AWS_DELETE_RECOVERY_DAYS must be an integer between 7 and 30",
    );
  }
  ...
  return {
    region,
    endpoint:
      process.env.BULLPEN_SECRETS_AWS_ENDPOINT?.trim() ||
...
function getAwsConfigReadiness() {
  const region = (
    process.env.BULLPEN_SECRETS_AWS_REGION ??
    process.env.AWS_REGION ??
    process.env.AWS_DEFAULT_REGION
  )?.trim();
  const deploymentId = process.env.BULLPEN_SECRETS_AWS_DEPLOYMENT_ID?.trim();
  const kmsKeyId = process.env.BULLPEN_SECRETS_AWS_KMS_KEY_ID?.trim();
  const missingConfig: string[] = [];

  if (!region) {
    missingConfig.push("BULLPEN_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!deploymentId) {
    missingConfig.push("BULLPEN_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!kmsKeyId) {
    missingConfig.push("BULLPEN_SECRETS_AWS_KMS_KEY_ID");
  }
...
function loadAwsSecretsManagerConfig(): AwsSecretsManagerConfig {
  ...
  const region =
    process.env.BULLPEN_SECRETS_AWS_REGION?.trim() ||
  ...
  if (!region) {
    throw unprocessable(
      "AWS Secrets Manager provider requires BULLPEN_SECRETS_AWS_REGION or AWS_REGION",
    );
  }
  if (!deploymentId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires BULLPEN_SECRETS_AWS_DEPLOYMENT_ID",
    );
  }
  if (!kmsKeyId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires BULLPEN_SECRETS_AWS_KMS_KEY_ID",
    );
  }
  ...
  return {
    ...
    endpoint:
      ...
      `https://secretsmanager.${region}.amazonaws.com`,
    deploymentId,
    prefix: sanitizePathSegment(process.env.BULLPEN_SECRETS_AWS_PREFIX?.trim() || DEFAULT_PREFIX),
    kmsKeyId,
    environmentTag:
      process.env.BULLPEN_SECRETS_AWS_ENVIRONMENT?.trim() ||
      process.env.NODE_ENV?.trim() ||
      "unknown",
    providerOwnerTag:
      process.env.BULLPEN_SECRETS_AWS_PROVIDER_OWNER?.trim() || DEFAULT_OWNER_TAG,
    deleteRecoveryWindowDays: recoveryWindow,
  };
}
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Bullpen-managed namespace secrets cannot be imported as external references",
  );
}
...
function buildManagedSecretTags(
  ...
  return [
    ...
    { Key: "bullpen:provider-owner", Value: config.providerOwnerTag },
    { Key: "bullpen:deployment-id", Value: config.deploymentId },
    { Key: "bullpen:company-id", Value: context.companyId },
    { Key: "bullpen:secret-key", Value: context.secretKey },
    { Key: "bullpen:environment", Value: config.environmentTag },
  ];
}
...
function inferPathSignals(entry: AwsSecretsManagerListSecretEntry, tags: Map<string, string>) {
  ...
  const bullpenDeploymentId = tagValue(tags, ["bullpen:deployment-id"]);
  const bullpenManaged = tagValue(tags, ["bullpen:managed-by"])?.toLowerCase() === "bullpen";

  if (bullpenDeploymentId || bullpenManaged) {
    return {
      prefix: segments[0] ?? DEFAULT_PREFIX,
      namespace: bullpenDeploymentId ?? segments[1] ?? null,
    };
  }
...
function discoverAwsProviderConfigCandidates(input: {
  ...
  type DiscoverySample = {
    ...
    ownerTag: string | null;
    kmsKeyId: string | null;
    bullpenManaged: boolean;
    bullpenCompanyId: string | null;
  };

  const skippedWarnings: string[] = [];
  let skippedForeignBullpenSampleCount = 0;
  ...
  for (const entry of input.entries) {
    ...
    const bullpenManaged = tagValue(tags, ["bullpen:managed-by"])?.toLowerCase() === "bullpen";
    const bullpenCompanyId = tagValue(tags, ["bullpen:company-id"]);
    if (bullpenManaged && bullpenCompanyId !== input.companyId) {
    ...
    samples.push({
      ...
      environmentTag: tagValue(tags, ["bullpen:environment", "environment", "env", "stage"]),
      ownerTag: tagValue(tags, ["bullpen:provider-owner", "owner", "team", "service", "application"]),
  ...
  if (skippedForeignBullpenSampleCount > 0) {
    skippedWarnings.push(
      `Skipped ${skippedForeignBullpenSampleCount} Bullpen-managed AWS secret sample(s) that were not tagged for this company.`,
    );
  }
  ...
  const candidates = [...groups.values()]
    ...
    .map((group) => {
      ...
      if (group.some((sample) => sample.bullpenManaged && sample.bullpenCompanyId === input.companyId)) {
        candidateWarnings.push("Sample includes Bullpen-managed secrets for this company; do not import them as external references.");
      }
      ...
      return {
        ...
        signals: {
          ...
          sampleCount: group.length,
          bullpenManagedSampleCount: group.filter((sample) => sample.bullpenManaged).length,
          skippedForeignBullpenSampleCount,
        },
...
export function createAwsSecretsManagerProvider(
  ...
  async function validateConfig(
    ...
    if (!config.prefix) {
      warnings.push("BULLPEN_SECRETS_AWS_PREFIX should be set to a deployment-scoped prefix");
    }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Bullpen metadata separately from AWS-managed secrets.",
          "Restoring access requires the Bullpen database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
      ...
      return {
        ...
        details: {
          ...
          optionalProviderConfig: [
            "BULLPEN_SECRETS_AWS_PREFIX",
            "BULLPEN_SECRETS_AWS_ENVIRONMENT",
            "BULLPEN_SECRETS_AWS_PROVIDER_OWNER",
            "BULLPEN_SECRETS_AWS_ENDPOINT",
            "BULLPEN_SECRETS_AWS_DELETE_RECOVERY_DAYS",
          ],
          credentialSource: "AWS SDK default credential provider chain",
          detectedCredentialSources: readiness.credentialSources,
        },
  ...
  return {
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Bullpen secret ${input.context.secretName}` : undefined,
    ...
    async createVersion(input) {
      ...
      try {
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
          VersionStages: [BULLPEN_PENDING_VERSION_STAGE],
        });
        const normalizedSecretId = created.ARN ?? created.Name ?? secretId;
        return {
          material: createManagedMaterial(normalizedSecretId, created.VersionId ?? null),
    ...
    async updateExternalSecretValue(input) {
      ...
      try {
        ...
        // referenced secret (not just Bullpen) picks up the new value.
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
        });
      ...
      try {
        if (input.mode === "archive") {
          if (material.versionId && gateway.updateSecretVersionStage) {
            await gateway.updateSecretVersionStage({
              SecretId: secretId,
              VersionStage: BULLPEN_PENDING_VERSION_STAGE,
