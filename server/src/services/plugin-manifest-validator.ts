 ...
 */
import { pluginManifestV1Schema } from "@bullpen/shared";
import type { BullpenPluginManifestV1 } from "@bullpen/shared";
import { PLUGIN_API_VERSION } from "@bullpen/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): BullpenPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as BullpenPluginManifestV1,
        };
      }
