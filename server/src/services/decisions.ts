...
import { companyMemberships, decisionBundles, decisionEffectExecutions, decisions, decisionTargetIssues, heartbeatRuns, issueRelations, issues } from "@bullpen/db";
import type { DecisionEffect, DecisionInput, DecisionOption, DecisionStatsCounts, DecisionStatsResponse } from "@bullpen/shared";
...
export function decisionService(db: Db, options: DecisionServiceOptions) {
  ...
  async function createInStore(input: CreateInput, dbOrTx: Db) {
    ...
    const cap = Number(process.env.BULLPEN_DECISIONS_OPEN_CAP ?? 50);
  ...
  async function sweepExpired(now = new Date()) {
    const configuredBatchSize = Number(process.env.BULLPEN_DECISIONS_SWEEP_BATCH_SIZE ?? 100);
    const batchSize = Number.isFinite(configuredBatchSize)
      ? Math.max(1, Math.trunc(configuredBatchSize))
      : 100;
    const configuredRecoveryGraceMs = Number(process.env.BULLPEN_DECISIONS_RECOVERY_GRACE_MS ?? 60_000);
