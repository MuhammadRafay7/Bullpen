...
import { claudeConfigDir, parseClaudeStreamJson } from "@bullpen/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@bullpen/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@bullpen/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveBullpenInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "bullpen-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "bullpen-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "bullpen-feedback-v1";
const FEEDBACK_DESTINATION = "bullpen_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "bullpen_run" &&
    file.source !== "bullpen_run_events" &&
    file.source !== "bullpen_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.BULLPEN_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readBullpenSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let bullpenRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      bullpenRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "bullpen/run.json",
        contentType: "application/json",
        source: "bullpen_run",
        contents: `${JSON.stringify(bullpenRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.bullpenRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "bullpen/run-events.json",
        contentType: "application/json",
        source: "bullpen_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "bullpen/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "bullpen_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.bullpenRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
