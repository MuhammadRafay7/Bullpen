...
type ToolActionSigningSecretEnv = Partial<
  Record<"BULLPEN_TOOL_ACTION_SIGNING_SECRET" | "BULLPEN_AGENT_JWT_SECRET" | "BETTER_AUTH_SECRET", string | undefined>
>;
...
export class ToolActionSigningSecretMissingError extends Error {
  constructor() {
    super(
      "BULLPEN_TOOL_ACTION_SIGNING_SECRET is not configured; signed tool action approvals cannot be issued. " +
        "Set BULLPEN_TOOL_ACTION_SIGNING_SECRET in this instance's environment (worktrees inherit it from .bullpen/.env).",
    );
...
export function resolveToolActionSigningSecret(env: ToolActionSigningSecretEnv = process.env as ToolActionSigningSecretEnv) {
  const secret = env.BULLPEN_TOOL_ACTION_SIGNING_SECRET?.trim();
  if (!secret) {
