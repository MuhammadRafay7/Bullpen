 ...
 * ~/.bullpen/adapter-plugins.json. This is the source of truth for which
...
import { resolveBullpenHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-bullpen-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const bullpenDir = resolveBullpenHomeDir();
  return {
    adapterPluginsDir: path.join(bullpenDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(bullpenDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(bullpenDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "bullpen-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Bullpen external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
