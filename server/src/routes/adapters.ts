 ...
 * toggle server-side adapter code for the whole Bullpen instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-bullpen-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-bullpen-adapter@0.3.0" → packageName + version
  ...
  // without modifying Bullpen's source code.
  //
