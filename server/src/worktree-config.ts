...
import type { BullpenConfig } from "@bullpen/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Bullpen worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.BULLPEN_IN_WORKTREE !== "true") return null;

  const configPath = resolveBullpenConfigPath(overrideConfigPath);
  const envPath = resolveBullpenEnvPath(configPath);
  const persistedEnv = readEnvEntries(envPath);

  // BULLPEN_IN_WORKTREE can leak in from a parent process or a sourced env
  ...
  // a target as a worktree when its config sits in a `<root>/.bullpen/`
  ...
  if (path.basename(path.dirname(configPath)) !== ".bullpen") return null;
  if (persistedEnv.BULLPEN_IN_WORKTREE !== "true") return null;

  const persistedConfigPath = nonEmpty(persistedEnv.BULLPEN_CONFIG);
  ...
  const worktreeName =
    nonEmpty(stablePersistedEnv.BULLPEN_WORKTREE_NAME) ??
    nonEmpty(env.BULLPEN_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(stablePersistedEnv.BULLPEN_INSTANCE_ID) ??
    nonEmpty(env.BULLPEN_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(stablePersistedEnv.BULLPEN_HOME) ??
      nonEmpty(env.BULLPEN_HOME) ??
      nonEmpty(env.BULLPEN_WORKTREES_DIR) ??
      "~/.bullpen-worktrees",
  );
...
function writeConfigFile(configPath: string, config: BullpenConfig): void {
...
function collectSiblingWorktreePorts(
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".bullpen", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as BullpenConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: BullpenConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: BullpenConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.BULLPEN_HOME = context.homeDir;
  process.env.BULLPEN_INSTANCE_ID = context.instanceId;
  process.env.BULLPEN_CONFIG = context.configPath;
  process.env.BULLPEN_CONTEXT = context.contextPath;
  process.env.BULLPEN_WORKTREE_NAME = context.worktreeName;
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const runtimeConfig = withWorktreePortRegistryLock(context.homeDir, () => {
        const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as BullpenConfig;
  ...
  const managedEnvEntries: Record<string, string> = {
    BULLPEN_HOME: context.homeDir,
    BULLPEN_INSTANCE_ID: context.instanceId,
    BULLPEN_CONFIG: context.configPath,
    BULLPEN_CONTEXT: context.contextPath,
    BULLPEN_IN_WORKTREE: "true",
    BULLPEN_DB_BACKUP_ENABLED: "false",
    BULLPEN_WORKTREE_NAME: context.worktreeName,
  };

  process.env.BULLPEN_DB_BACKUP_ENABLED = "false";
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: BullpenConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as BullpenConfig;
  } catch {
    return;
  }
