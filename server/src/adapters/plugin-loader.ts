...
function extractUiParserSource(
  ...
  const contractVersion = pkg.bullpen?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no bullpen.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
