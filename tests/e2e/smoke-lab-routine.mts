 ...
 * Bullpen company — provided in every routine run's env):
 *   BULLPEN_API_URL, BULLPEN_API_KEY, BULLPEN_COMPANY_ID, BULLPEN_RUN_ID
 *   ROUTINE_ISSUE_ID=<uuid>            issue to record against (default BULLPEN_TASK_ID)
...
const CP_BASE = (() => {
  const b = (process.env.BULLPEN_API_URL ?? "").replace(/\/$/, "");
  return b.replace(/\/api$/, "");
})();
const CP_KEY = process.env.BULLPEN_API_KEY ?? "";
const CP_COMPANY = process.env.BULLPEN_COMPANY_ID ?? "";
const CP_RUN = process.env.BULLPEN_RUN_ID ?? "";
const ROUTINE_ISSUE_ID = process.env.ROUTINE_ISSUE_ID ?? process.env.BULLPEN_TASK_ID ?? "";
...
async function cpUploadAsset(file: string): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch(`${CP_BASE}/api/companies/${CP_COMPANY}/assets/images`, {
      method: "POST",
      headers: { authorization: `Bearer ${CP_KEY}`, "x-bullpen-run-id": CP_RUN },
      body: form as any,
    });
    if (!res.ok) {
