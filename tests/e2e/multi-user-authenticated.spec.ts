...
const BASE = process.env.BULLPEN_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.BULLPEN_E2E_DATA_DIR ?? process.env.BULLPEN_HOME;
const CONFIG_PATH = process.env.BULLPEN_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".bullpen/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "bullpen-owner-password";
const INVITED_PASSWORD = "bullpen-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@bullpen.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@bullpen.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("BULLPEN_E2E_DATA_DIR or BULLPEN_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Bullpen" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Bullpen account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Bullpen instance" })).toBeVisible();
