...
Run this against a dev Bullpen instance with a board token or an agent token that can manage pipelines, routines, and issues.

```sh
export BULLPEN_API_URL=http://localhost:3100
export BULLPEN_COMPANY_ID=<company-id>
export BULLPEN_API_KEY=<token>
...
bullpen pipelines create \
  -C "$BULLPEN_COMPANY_ID" \
...
bullpen pipelines set-transitions \
  -C "$BULLPEN_COMPANY_ID" \
  "$RELEASE_PIPELINE" \
...
bullpen pipelines guidance put \
  -C "$BULLPEN_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
...
export DRAFTING_ROUTINE_ID="$(
  bullpen routine create \
    -C "$BULLPEN_COMPANY_ID" \
    --payload-json "$(jq -c . /tmp/drafting-routine.json)" \
    --json | jq -r '.id'
)"

bullpen pipelines set-automation \
  -C "$BULLPEN_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
  --stage drafting \
...
bullpen pipelines ingest-batch \
  -C "$BULLPEN_COMPANY_ID" \
  "$FEATURE_PIPELINE" \
...
export SUGGESTION_ID="$(
  bullpen pipelines case suggest \
    -C "$BULLPEN_COMPANY_ID" \
    "$BLOG_CASE" \
    --to assets \
...
bullpen pipelines case resolve-suggestion \
  -C "$BULLPEN_COMPANY_ID" \
  "$BLOG_CASE" \
...
curl -sS -X POST \
  ...
  "$BULLPEN_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null

bullpen pipelines case edit \
  -C "$BULLPEN_COMPANY_ID" \
  "$BLOG_CASE" \
  --expected-version 2 \
...
# Recovery:
bullpen pipelines case get -C "$BULLPEN_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(bullpen pipelines case get -C "$BULLPEN_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(bullpen pipelines case get -C "$BULLPEN_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

bullpen pipelines case transition \
  -C "$BULLPEN_COMPANY_ID" \
  "$BLOG_CASE" \
  --to assembly \
...
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(bullpen pipelines case get -C "$BULLPEN_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

bullpen pipelines case review \
  -C "$BULLPEN_COMPANY_ID" \
  "$BLOG_CASE" \
  --approve \
...
```sh
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
bullpen pipelines case review -C "$BULLPEN_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
bullpen pipelines case transition -C "$BULLPEN_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
...
```sh
BULLPEN_API_URL=http://localhost:3100 \
BULLPEN_COMPANY_ID=<company-id> \
BULLPEN_API_KEY=<token> \
