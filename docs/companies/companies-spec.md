...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Bullpen.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.bullpen.yaml`
...
- Bullpen must not require extra top-level fields for skill validity
- Bullpen-specific extensions must live under `metadata.bullpen` or `metadata.sources`
...
For Bullpen, the preferred fidelity extension is:
...
- issue/task Bullpen-only metadata
...
- Bullpen tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Bullpen shape:
...
Additional rules for Bullpen exporters:
...
- omit empty and default-valued Bullpen fields when possible
...
- Bullpen exporters should emit `.bullpen.yaml` as a sidecar by default
...
Bullpen can map this spec to its runtime model like this:
...
- Bullpen extension:
  - `.bullpen.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Bullpen-specific fidelity

Inline Bullpen-only metadata that must live inside a shared markdown file should use:

- `metadata.bullpen`

That keeps the base format broader than Bullpen.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Bullpen.

## 20. Cutover

Bullpen should cut over to this markdown-first package model as the primary portability format.

`bullpen.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Bullpen, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
