# ClipHub: Marketplace for Bullpen Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Bullpen configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Bullpen-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Bullpen company (agents, projects, governance) |
...
A complete Bullpen company configuration:
...
Single-agent configurations designed to plug into a Bullpen org:
...
Portable skill files that any Bullpen agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Bullpen's skill loading system
...
interface Listing {
  ...
  bullpenVersionMin: string;     // Minimum Bullpen version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Bullpen company
...
| `POST` | `/api/listings/:id/install` | Install to Bullpen company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Bullpen company (or create new)
  → ClipHub API calls Bullpen API to:
  ...
  → Redirect to Bullpen dashboard with new team running
```
...
### 5.3 Creator: Export from Bullpen → Publish

```
Running Bullpen company → "Export as Blueprint" (CLI or UI)
  → Bullpen exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Bullpen brand blue/purple)
...
| Install flow | One-click deploy to Bullpen company |
...
{
  "targetCompanyId": "uuid",         // Existing Bullpen company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `bullpen-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Bullpen
- **Backend**: Node.js API (or extend Bullpen server)
- **Database**: Postgres (can share Bullpen's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Bullpen auth (or OAuth2)

### 9.2 Integration with Bullpen

ClipHub can be:
- **Option A**: A separate app that calls Bullpen's API to install blueprints
- **Option B**: A built-in section of the Bullpen UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Bullpen UI and API.
...
- [ ] Install flow: blueprint → Bullpen company
...
- [ ] "Export from Bullpen" CLI command
