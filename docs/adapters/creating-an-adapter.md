...
Build a custom adapter to connect Bullpen to any agent runtime.
...
|---|---|---|
| Source | Inside `bullpen-fork` | Separate npm package |
| Distribution | Ships with Bullpen | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Bullpen's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@bullpen/adapter-utils/server-utils`
2. Build environment with `buildBullpenEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@bullpen/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildBullpenEnv(agent)` | `@bullpen/adapter-utils/server-utils` | Inject `BULLPEN_*` env vars |
| `renderTemplate(tpl, data)` | `@bullpen/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@bullpen/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@bullpen/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Bullpen's source:
...
`format-event.ts` — pretty-prints stdout for `bullpen run --watch` using `picocolors`.
...
With these flags set, the Bullpen UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Bullpen source changes required.
...
Make Bullpen skills discoverable to your agent runtime without writing to the agent's working directory:
