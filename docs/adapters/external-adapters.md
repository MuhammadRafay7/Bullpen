...
summary: Build, package, and distribute adapters as plugins without modifying Bullpen source
---

Bullpen supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Bullpen's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `bullpen-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Bullpen | Published to npm or linked via `file:` |
| Updates | Requires Bullpen release | Independent versioning |
...
- `hermes_local` starts the local Hermes CLI from
  `@bullpen/hermes-bullpen-adapter`.
- `hermes_gateway` calls an already-running Hermes API server through
  `@bullpen/hermes-bullpen-adapter/gateway`.

The legacy `@bullpen/adapter-hermes-gateway` package is a deprecated
...
depend on or link `@bullpen/hermes-bullpen-adapter` and declare the type
...
```json
{
  ...
  "dependencies": {
    "@bullpen/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `bullpen.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@bullpen/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Bullpen vars injected
  const env = buildBullpenEnv(agent);
...
#### Available Helpers from `@bullpen/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildBullpenEnv(agent)` | Inject `BULLPEN_*` environment variables |
...
```sh
# Via the Bullpen UI
# Settings → Adapters → Install from npm → "my-bullpen-adapter"
...
Local adapters are symlinked into Bullpen's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.bullpen/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-bullpen-adapter",
...
Other Bullpen users can then install your adapter by package name from the UI or API.
