...
Bullpen `external_reference` secrets without copying plaintext into
Bullpen.
...
`ListSecrets` only and import stores a Bullpen external reference plus
...
remote metadata, or copies secret plaintext into Bullpen.
...
- `conflict`: the suggested Bullpen `name` or `key` is already in use.
...
Bullpen-managed AWS secret through a broad runtime role.
...
Each row may include an optional Bullpen `description` entered during review;
...
the Bullpen runtime role can list the AWS secret but lacks
