...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Bullpen-Signature` + `X-Bullpen-Timestamp` header pair matching the trigger's signing mode.
