...
summary: Deploy Bullpen to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Bullpen to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Bullpen repo cloned locally
...
export BULLPEN_DOMAIN=bullpen.example.com   # your domain
...
aws ecr create-repository \
  --repository-name bullpen-server \
...
# Build
docker build -t bullpen-server .

# Tag and push
docker tag bullpen-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/bullpen-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name bullpen-alb \
  --description "Bullpen ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name bullpen-ecs \
  --description "Bullpen ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name bullpen-rds \
  --description "Bullpen RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name bullpen-efs \
  --description "Bullpen EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name bullpen-db-subnet \
  --db-subnet-group-description "Bullpen RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier bullpen-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username bullpen \
...
DATABASE_URL="postgresql://bullpen:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/bullpen"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=bullpen-data \
...
aws secretsmanager create-secret \
  --name bullpen/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name bullpen/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name bullpen/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name bullpen/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name bullpen/github-token \
...
# Task execution role
aws iam create-role \
  --role-name bullpen-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:bullpen/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name bullpen-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name bullpen

aws logs create-log-group --log-group-name /ecs/bullpen
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$BULLPEN_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/bullpen-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/bullpen-task-def.json
```
...
- Create a CNAME or ALIAS record for `$BULLPEN_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster bullpen \
  --service-name bullpen-server \
  --task-definition bullpen-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster bullpen \
  --services bullpen-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster bullpen --service-name bullpen-server
TASK_ARN=$(aws ecs list-tasks --cluster bullpen --service-name bullpen-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster bullpen --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/bullpen --since 10m --follow

# Hit the health endpoint
curl -sf https://$BULLPEN_DOMAIN/api/health
```
...
aws ecs update-service \
  --cluster bullpen \
  --service bullpen-server \
  --task-definition bullpen-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier bullpen-db
aws rds start-db-instance --db-instance-identifier bullpen-db
```
...
aws ecs update-service --cluster bullpen --service bullpen-server --desired-count 0
aws ecs delete-service --cluster bullpen --service bullpen-server --force
aws ecs delete-cluster --cluster bullpen
...
aws rds delete-db-instance \
  --db-instance-identifier bullpen-db \
  --final-db-snapshot-identifier bullpen-db-final
aws rds wait db-instance-deleted --db-instance-identifier bullpen-db
aws rds delete-db-subnet-group --db-subnet-group-name bullpen-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id bullpen/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name bullpen-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name bullpen-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name bullpen-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name bullpen-ecs-execution
aws iam delete-role --role-name bullpen-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/bullpen
```
