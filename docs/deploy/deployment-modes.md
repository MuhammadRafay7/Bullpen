...
Bullpen supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm bullpen allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Bullpen emits a one-time claim URL at startup:
...
```sh
BULLPEN_DEPLOYMENT_MODE=authenticated BULLPEN_BIND=lan pnpm bullpen run
```
