...
Bullpen encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Bullpen protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Bullpen does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Bullpen can no longer guarantee
...
At runtime, Bullpen resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Bullpen injects its
own `BULLPEN_*` runtime variables.
...
Bullpen stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from Bullpen secrets.
...
- Keep Bullpen-managed AWS values under the Bullpen managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  Bullpen-managed flow when Bullpen should create and rotate the value.

### IAM Caveats

Bullpen enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the Bullpen runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the Bullpen managed prefix,
...
refs. Bullpen can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside Bullpen.
...
```
~/.bullpen/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
Bullpen best-effort enforces `0600` permissions when it creates or loads the
key file. `bullpen doctor` and the provider health API warn when the file is
...
|----------|-------------|
| `BULLPEN_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `BULLPEN_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `BULLPEN_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `BULLPEN_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into Bullpen by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Bullpen stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Bullpen Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Bullpen company secrets**. Allowed credential sources are
workload identity attached to the Bullpen server (instance profile, IRSA, ECS
...
can override) the deployment-level `BULLPEN_SECRETS_AWS_*` env. Bootstrap
...
Bullpen `external_reference` secrets. This is a metadata-only link: Bullpen
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Bullpen name and key, then
   submit.
...
cursors. Do not expect Bullpen to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Bullpen runtime
...
- Keep managed create/rotate/delete permissions scoped to the Bullpen
...
dedicated Bullpen runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Bullpen
...
- `aws_secrets_manager`: back up Bullpen's database for vault metadata
  ...
  same Bullpen company at the same AWS namespace and confirming the runtime
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Bullpen. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Bullpen
...
For Bullpen Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `BULLPEN_SECRETS_AWS_*` environment
...
Bullpen secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
pnpm bullpen secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the Bullpen
...
the value custodian; Bullpen stores metadata and provider version references,
