...
The **Skills Store** is Bullpen's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with Bullpen | The `@bullpen/skills-catalog` package |
...
- **`bundled`** — first-party Bullpen skills (e.g. `issue-triage`, `task-planning`,
  `qa-acceptance`, `wireframe`, `github-pr-workflow`, `doc-maintenance`). These carry the
  reserved `bullpen/bullpen/...` key namespace.
...
- **`category`** — grouping such as `software-development`, `quality`, `product`,
  `research`, `content`, `browser`, `bullpen-operations`, `docs`.
...
|---|---|---|
| `catalog` | Bullpen / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: Bullpen ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when Bullpen can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
Bullpen approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and Bullpen fetches and imports it. Accepted forms include:
...
The catalog is its own publishable package, `@bullpen/skills-catalog`:
