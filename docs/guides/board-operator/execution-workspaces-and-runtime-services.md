...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Bullpen.

Bullpen now presents this as a workspace-command model:
...
- Bullpen does not automatically start or stop these workspace services as part of issue execution.
- Bullpen also does not automatically restart workspace services on server boot.
...
2. Bullpen realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Bullpen persists execution-workspace metadata such as paths, refs, and provisioning settings.
