...
Use `hermes_local` when Bullpen should start the local Hermes CLI. Use
`hermes_gateway` when Hermes is already running as an API server and Bullpen
should call that server. Both are built-in adapter types from the unified
`@bullpen/hermes-bullpen-adapter` package.

For `opencode_local`, configure an explicit `adapterConfig.model` (`provider/model`).
Bullpen validates the selected model against live `opencode models` output.
