...
Add first-class **documents** to Bullpen as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Bullpen workflow/docs.
...
Update the Bullpen agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/bullpen/SKILL.md`
