...
usage() {
  cat <<'EOF'
Usage:
  bullpen-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current Bullpen
issue, then creates an attachment-backed artifact work product by default.

Required environment for live uploads:
  BULLPEN_API_URL, BULLPEN_API_KEY, BULLPEN_COMPANY_ID, BULLPEN_TASK_ID, BULLPEN_RUN_ID

Options:
  --issue-id ID          Issue id to attach to (default: BULLPEN_TASK_ID)
  --company-id ID        Company id (default: BULLPEN_COMPANY_ID)
...
Examples:
  scripts/bullpen-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  scripts/bullpen-upload-artifact.sh out/walkthrough.webm \
...
file_path=""
issue_id="${BULLPEN_TASK_ID:-}"
company_id="${BULLPEN_COMPANY_ID:-}"
title=""
summary=""
content_type=""
...
if [[ -z "${BULLPEN_API_URL:-}" || -z "${BULLPEN_API_KEY:-}" || -z "${BULLPEN_RUN_ID:-}" ]]; then
  printf 'Missing BULLPEN_API_URL, BULLPEN_API_KEY, or BULLPEN_RUN_ID.\n' >&2
  exit 1
fi

if [[ -z "$issue_id" || -z "$company_id" ]]; then
  printf 'Missing issue or company id. Pass --issue-id/--company-id or set BULLPEN_TASK_ID/BULLPEN_COMPANY_ID.\n' >&2
  exit 1
fi
