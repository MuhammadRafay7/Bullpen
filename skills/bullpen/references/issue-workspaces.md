...
```sh
curl -sS -H "Authorization: Bearer $BULLPEN_API_KEY" \
  "$BULLPEN_API_URL/api/issues/$BULLPEN_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so Bullpen preserves workspace continuity.

## Control Services

Prefer Bullpen-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.
...
curl -sS -X POST \
  ...
  "$BULLPEN_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Bullpen-Run-Id: $BULLPEN_RUN_ID" \
  -H "Content-Type: application/json" \
  "$BULLPEN_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Bullpen-Run-Id: $BULLPEN_RUN_ID" \
  -H "Content-Type: application/json" \
  "$BULLPEN_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the Bullpen MCP tools are available, prefer these issue-scoped tools:

- `bullpenGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `bullpenControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `bullpenWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
