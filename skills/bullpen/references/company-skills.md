...
- App-shipped catalog: a curated set of company skills in `@bullpen/skills-catalog`, browseable and installable without leaving Bullpen.
...
`bullpen skills` CLI documented in `doc/CLI.md` — it wraps every endpoint
...
```sh
curl -sS "$BULLPEN_API_URL/api/skills/catalog?kind=bundled" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"

curl -sS "$BULLPEN_API_URL/api/skills/catalog/ref?ref=github-pr-workflow" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"

curl -sS -X POST "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills/install-catalog" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "catalogSkillId": "bullpen:bundled:software-development:github-pr-workflow"
  }'
```
...
```sh
curl -sS -X POST "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"
```
...
```sh
curl -sS "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"

curl -sS "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"
```
...
```sh
curl -sS -X POST "$BULLPEN_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$BULLPEN_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"
```
...
```sh
curl -sS -X POST "$BULLPEN_API_URL/api/companies/$BULLPEN_COMPANY_ID/agents" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Bullpen runtime skills are still added automatically when required by the adapter.
