...
description: >
  Create new agents in Bullpen with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Bullpen Create Agent Skill
...
```sh
curl -sS "$BULLPEN_API_URL/api/agents/me" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"
```

### 2. Discover adapter configuration for this Bullpen instance
...
`skills/bullpen-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/bullpen-create-agent/references/baseline-role-guide.md`
...
`skills/bullpen-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `BULLPEN_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$BULLPEN_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"

curl -sS -X POST "$BULLPEN_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$BULLPEN_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $BULLPEN_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$BULLPEN_API_URL/api/approvals/$BULLPEN_APPROVAL_ID" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"

curl -sS "$BULLPEN_API_URL/api/approvals/$BULLPEN_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $BULLPEN_API_KEY"
```
...
- Template index and how to apply a template: `skills/bullpen-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/bullpen-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/bullpen-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/bullpen-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/bullpen-create-agent/references/api-reference.md`
