...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@bullpen/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@bullpen/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@bullpen/adapter-utils").TranscriptEntry[];
