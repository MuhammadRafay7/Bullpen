import type { ActivityEvent, Issue, SuccessfulRunHandoffState } from "@bullpen/shared";
...
export function isSuccessfulRunHandoffEscalationComment(text: string) {
  ...
  return trimmed === SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY
    || /^Bullpen exhausted the bounded successful-run handoff correction\b/i.test(trimmed);
}
