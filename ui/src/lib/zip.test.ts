...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("bullpen-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("bullpen-demo/COMPANY.md");
  ...
  it("reads a Bullpen zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
  ...
  it("reads standard DEFLATE zip archives created outside Bullpen", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("bullpen-demo");
...
describe("estimateZipArchiveSize", () => {
  it("matches the real archive byte length for mixed text and binary entries", () => {
    ...
    expect(estimateZipArchiveSize(files, "bullpen-demo")).toBe(
      createZipArchive(files, "bullpen-demo").byteLength,
    );
  });
  ...
  it("returns the 22-byte end-of-central-directory record for an empty map", () => {
    expect(estimateZipArchiveSize({}, "bullpen-demo")).toBe(22);
    expect(estimateZipArchiveSize({}, "bullpen-demo")).toBe(
      createZipArchive({}, "bullpen-demo").byteLength,
    );
  });
});
