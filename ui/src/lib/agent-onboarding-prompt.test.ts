...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Bullpen API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.bullpenApiUrl");
    ...
    expect(prompt).toContain("Bullpen maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Bullpen host");
    expect(prompt).toContain("Hermes-originated Bullpen API calls");
    expect(prompt).toContain("Do not rotate or invent a Bullpen key manually");
