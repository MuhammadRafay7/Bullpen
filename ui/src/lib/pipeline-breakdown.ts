...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Bullpen will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Bullpen handles this" mechanics bullets, composed from the
