 ...
 *
 * Plugin bundles are built with `external: ["@bullpen/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__bullpenPluginBridge__`)
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__bullpenPluginBridge__?.react;
        if (!R) {
          throw new Error("Bullpen plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__bullpenPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__bullpenPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingBullpenSdkUiComponent() {
            throw new Error('Bullpen plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@bullpen/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@bullpen/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@bullpen/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@bullpen/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@bullpen/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__bullpenPluginBridge__) {
 ...
 * self-register via `window.bullpenPlugins.registerReactComponent()`.
 ...
 * of `@bullpen/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
