...
import type { RoutineListItem, RoutineVariable } from "@bullpen/shared";
import type { FolderListItem } from "@bullpen/shared";
import {
  AllUnfiledBanner,
  BulkBar,
  DeleteFolderDialog,
...
export function Routines() {
  ...
  const routineViewStateKey = selectedCompanyId
    ? `bullpen:routines-view:${selectedCompanyId}`
    : "bullpen:routines-view";
  ...
  const createFolder = useMutation({
      ...
      if (moveAfterCreateIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            ...
            body: moveError instanceof Error ? moveError.message : "Bullpen could not move the selected routines.",
            tone: "error",
          });
          return;
        }
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not save the folder.",
        tone: "error",
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not update the folder.",
        tone: "error",
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not delete the folder.",
        tone: "error",
      });
    },
  });
  const moveRoutineToFolder = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: "Move failed",
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not move the routine.",
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not update the routine.",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : "Bullpen could not start the routine run.",
        tone: "error",
      });
    },
  });
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="runs">
          <IssuesList
            ...
            viewStateKey="bullpen:routine-recent-runs-view"
        ...
        >
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              After creation, Bullpen takes you straight to trigger setup. Draft routines stay paused until you add a default agent.
            </div>
      ...
      {activeTab === "routines" ? (
        <div className={cn(showFolderRail && "flex gap-4")}>
          ...
          {routineViewState.groupBy === "folder" && !hasRoutineFolders && !foldersLoading && visibleRoutines.length > 0 ? (
            <AllUnfiledBanner
              storageKey={`bullpen:routines-folder-nudge:${selectedCompanyId ?? "none"}`}
              itemLabelPlural="routines"
