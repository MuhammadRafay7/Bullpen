...
function applyObservabilityHeaders(headers: Headers) {
  if (headers.has("X-Bullpen-Tab-Visible")) return; // caller override wins
  const visibility = getPageVisibility();
  headers.set("X-Bullpen-Tab-Visible", getVisibilityHeaderValue(visibility));
  if (typeof window !== "undefined" && window.location) {
    headers.set("X-Bullpen-Route", window.location.pathname);
  }
}
