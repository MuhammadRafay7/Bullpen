...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, skill, and routine mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Bullpen App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")}) [/routine:Weekly review](${buildRoutineMentionHref("routine-123")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
    ...
    expect(html).toContain("--bullpen-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--bullpen-mention-project-color:#336699");
  ...
  it("linkifies bare issue identifiers in markdown text", () => {
    ...
    expect(html).toContain("bullpen-markdown-issue-ref");
    expect(html).not.toContain("bullpen-mention-chip--issue");
  });
  ...
  it("preserves absolute issue URLs as external links", () => {
    ...
    expect(html).not.toContain("bullpen-markdown-issue-ref");
  });
  ...
  it("renders linked inline-code workspace paths as file viewer links before issue links", () => {
    const html = renderMarkdown(
      "- **MP4**: [`videos/90-days-bullpen/out/90-days-bullpen-1x1.mp4`](/PAP/issues/PAP-10306 \"Publish handoff\")",
    ...
    expect(html).toContain('data-workspace-file-path="videos/90-days-bullpen/out/90-days-bullpen-1x1.mp4"');
    expect(html).toContain("videos/90-days-bullpen/out/90-days-bullpen-1x1.mp4");
  ...
  it("leaves wiki links as text unless explicitly enabled", () => {
    const html = renderMarkdown("See [[wiki/entities/bullpen]].");

    expect(html).toContain("[[wiki/entities/bullpen]]");
    expect(html).not.toContain('href="/wiki/page/wiki/entities/bullpen.md"');
  });

  it("renders wiki links with a custom resolver when enabled", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/bullpen|Bullpen]] and [[wiki/entities/dotta-b]].",
      [],
      {
    ...
    expect(html).toContain('href="/wiki/page/wiki/entities/bullpen.md"');
    expect(html).toContain('data-bullpen-wiki-link="true"');
    expect(html).toContain('data-bullpen-wiki-target="wiki/entities/bullpen"');
    expect(html).toContain(">Bullpen</a>");
    expect(html).toContain('href="/wiki/page/wiki/entities/dotta-b.md"');
    expect(html).toContain(">wiki/entities/dotta-b</a>");
    expect(html).not.toContain("[[wiki/entities/bullpen");
  });

  it("keeps wiki links as text when the custom resolver rejects them", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/bullpen]].",
      [],
      {
        enableWikiLinks: true,
    ...
    expect(html).toContain("[[wiki/entities/bullpen]]");
    expect(html).not.toContain('data-bullpen-wiki-link="true"');
    expect(html).not.toContain('href="/wiki/page/wiki/entities/bullpen"');
  });

  it("does not render wiki links inside code spans or code blocks", () => {
    const html = renderMarkdown(
      "Inline `[[wiki/entities/bullpen]]`.\n\n```md\n[[wiki/entities/dotta-b]]\n```",
      [],
      {
        enableWikiLinks: true,
        wikiLinkRoot: "/wiki/page",
  ...
  it("applies wrap-friendly styles to long inline content", () => {
    ...
    expect(html).toContain('class="bullpen-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden');
  ...
  it("renders markdown tables in a horizontally scrollable region", () => {
    ...
    expect(html).toContain('class="bullpen-markdown-table-scroll"');
  ...
  it("renders a copy button alongside fenced code blocks", () => {
    ...
    expect(html).toContain("bullpen-markdown-codeblock");
    expect(html).toContain("bullpen-markdown-codeblock-actions");
    expect(html).toContain("position:absolute;top:0.4rem;right:0.4rem;display:inline-flex");
    expect(html).toContain("bullpen-markdown-codeblock-wrap");
    expect(html).toContain('aria-label="Wrap lines"');
    expect(html).toContain("position:static;opacity:1;display:inline-flex");
    expect(html).toContain("bullpen-markdown-codeblock-copy");
  ...
  it("does not render a copy button on inline code", () => {
    ...
    expect(html).not.toContain("bullpen-markdown-codeblock-copy");
  });
