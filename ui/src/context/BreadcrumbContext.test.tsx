...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Bullpen", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Bullpen",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Bullpen");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Bullpen");
    expect(buildDocumentTitle([], null)).toBe("Bullpen");
  });
});
