#!/usr/bin/env bash
#
# Kill all local Bullpen dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all bullpen dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/bullpen(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.bullpen/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.bullpen/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.bullpen/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/bullpen*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.bullpen/instances/*/db/postmaster.pid
    "$sibling_root"/.bullpen/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Bullpen dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Bullpen dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Bullpen node processes..."
