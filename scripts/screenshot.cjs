#!/usr/bin/env node
/**
 * Screenshot utility for Bullpen UI.
 *
 * Reads the board token from ~/.bullpen/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/bullpen-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".bullpen/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.bullpen/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Bullpen origin only
