...
async function handleRequest(request) {
  if (request.method === "initialize") {
    ...
    process.stderr.write("bullpen-acp-echo-agent started\n");
    return {
      protocolVersion: 1,
      agentCapabilities: { loadSession: false, sessionCapabilities: { close: {} } },
      agentInfo: { name: "bullpen-acp-echo-agent", version: "1.0.0" },
    };
  }
    ...
    writeMessage({
      ...
      params: {
        ...
        update: {
          ...
          content: { type: "text", text: process.env.BULLPEN_ACPX_SPAWN_SMOKE ?? "missing" },
        },
      },
    });
