...
BULLPEN_API_URL="${BULLPEN_API_URL:-http://127.0.0.1:3100}"
API_BASE="${BULLPEN_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${BULLPEN_COMPANY_ID:-}}"
COMPANY_SELECTOR="${COMPANY_SELECTOR:-}"

RUN_SUFFIX="${HERMES_SMOKE_RUN_SUFFIX:-$(date +%Y%m%d-%H%M%S)-$$}"
HERMES_IMAGE="${HERMES_IMAGE:-bullpen-hermes-gateway-smoke:local}"
...
HERMES_CONTAINER_NAME="${HERMES_CONTAINER_NAME:-bullpen-hermes-gateway-smoke-${RUN_SUFFIX}}"
...
HERMES_SMOKE_STATE_DIR="${HERMES_SMOKE_STATE_DIR:-${TMPDIR:-/tmp}/bullpen-hermes-gateway-smoke-${RUN_SUFFIX}}"
HERMES_SMOKE_DIAG_DIR="${HERMES_SMOKE_DIAG_DIR:-${TMPDIR:-/tmp}/bullpen-hermes-gateway-e2e-diag-${RUN_SUFFIX}}"
...
BULLPEN_API_URL_FOR_HERMES="${BULLPEN_API_URL_FOR_HERMES:-http://host.docker.internal:3100}"
...
joins it to Bullpen as a hermes_gateway agent, wakes that agent on a smoke
issue, verifies the issue result, captures redacted diagnostics, and cleans up
Bullpen and Docker state unless HERMES_SMOKE_KEEP=1.

Required:
  BULLPEN_API_URL=http://127.0.0.1:3100
  BULLPEN_AUTH_HEADER='Bearer <board-token>'     # or BULLPEN_COOKIE

Common flags:
  COMPANY_ID=<uuid> or COMPANY_SELECTOR=<prefix|name|uuid>
  HERMES_VERSION=0.17.0
  HERMES_IMAGE=bullpen-hermes-gateway-smoke:local
  HERMES_GATEWAY_PORT=8642
  HERMES_GATEWAY_API_BASE_URL=http://127.0.0.1:8642
  HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642
  BULLPEN_API_URL_FOR_HERMES=http://host.docker.internal:3100
...
Mode notes:
  HERMES_GATEWAY_API_BASE_URL is the URL stored on the Bullpen adapter and
  must be reachable by the Bullpen server. HERMES_GATEWAY_PROBE_URL is the URL
  this operator shell uses for direct gateway checks. They can differ for Docker
  network and reverse-proxy smoke runs.

  Raw Hermes and Bullpen API keys are redacted from logs and diagnostic files.
...
AUTH_HEADERS=()
if [[ -n "${BULLPEN_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${BULLPEN_AUTH_HEADER}")
elif [[ -n "${BULLPEN_API_KEY:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: Bearer ${BULLPEN_API_KEY}")
fi
if [[ -n "${BULLPEN_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${BULLPEN_COOKIE}")
  BULLPEN_BROWSER_ORIGIN="${BULLPEN_BROWSER_ORIGIN:-${BULLPEN_API_URL%/}}"
  AUTH_HEADERS+=(-H "Origin: ${BULLPEN_BROWSER_ORIGIN}" -H "Referer: ${BULLPEN_BROWSER_ORIGIN}/")
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board/operator auth required. Set BULLPEN_COOKIE, BULLPEN_AUTH_HEADER, or a board-capable BULLPEN_API_KEY."
  fi
...
capture_diagnostics() {
  mkdir -p "$HERMES_SMOKE_DIAG_DIR"
  {
    ...
    echo "bullpenApiUrl=${BULLPEN_API_URL}"
    echo "bullpenApiUrlForHermes=${BULLPEN_API_URL_FOR_HERMES}"
  ...
  capture_container_logs
  capture_issue_diagnostics "$SMOKE_ISSUE_ID" "bullpen-smoke"
  capture_run_diagnostics "$RUN_ID" "bullpen-smoke"
}

cleanup_bullpen_state() {
...
start_container() {
  ...
  local args=(
    ...
    -e BULLPEN_API_URL="$BULLPEN_API_URL_FOR_HERMES"
    -e NO_COLOR=1
...
assert_fresh_container_state() {
  ...
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | sort | grep -E "^(HOME|HERMES_HOME|XDG_|API_SERVER_|BULLPEN_API_URL)=" || true' > "${HERMES_SMOKE_DIAG_DIR}/container-env.txt"
  ...
  if docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | grep -q "^BULLPEN_API_KEY="'; then
    fail "container unexpectedly has BULLPEN_API_KEY before join/key claim"
  fi
}

probe_container_to_bullpen() {
  log "probing container-to-Bullpen connectivity at ${BULLPEN_API_URL_FOR_HERMES}/api/health"
  if ! docker exec "$HERMES_CONTAINER_NAME" curl -fsS --max-time 8 "${BULLPEN_API_URL_FOR_HERMES%/}/api/health" > "${HERMES_SMOKE_DIAG_DIR}/container-bullpen-health.json"; then
    fail "Hermes container cannot reach Bullpen. Set BULLPEN_API_URL_FOR_HERMES to a URL reachable from inside Docker, or keep HERMES_DOCKER_ADD_HOST=1 for Linux host.docker.internal."
  fi
}
...
probe_gateway_readiness() {
  ...
  if [[ "$HERMES_GATEWAY_PROBE_URL" != "$HERMES_GATEWAY_API_BASE_URL" ]]; then
    log "Bullpen will store Hermes gateway URL ${HERMES_GATEWAY_API_BASE_URL}"
  fi
...
assert_direct_gateway_run() {
  ...
  payload="$(jq -nc \
    --arg marker "$marker" \
    --arg session "bullpen-smoke-direct-${RUN_SUFFIX}" \
    '{input: ("Reply with exactly " + $marker + " and no other text."), instructions: "You are running a Bullpen Hermes gateway smoke direct API assertion.", session_id: $session}')"
...
assert_stop_behavior_if_deterministic() {
  ...
  local payload
  payload="$(jq -nc \
    --arg session "bullpen-smoke-stop-${RUN_SUFFIX}" \
...
join_hermes_agent() {
  ...
  BULLPEN_API_URL_FOR_HERMES="$BULLPEN_API_URL_FOR_HERMES" \
  BULLPEN_AUTH_HEADER="${BULLPEN_AUTH_HEADER:-}" \
  BULLPEN_API_KEY="${BULLPEN_API_KEY:-}" \
  BULLPEN_COOKIE="${BULLPEN_COOKIE:-}" \
...
install_claimed_key_in_container() {
  log "placing newly claimed Bullpen key in container workspace"
  local key_file="${HERMES_SMOKE_STATE_DIR}/workspace/bullpen-claimed-api-key.json"
  ...
  chmod 644 "$key_file"
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'test -f /home/hermes/workspace/bullpen-claimed-api-key.json && test ! -e "$HERMES_HOME/host-sentinel.txt"'
}

patch_agent_instructions_with_claimed_key() {
  log "patching Hermes agent instructions with claimed Bullpen API context"
  api_request "GET" "/agents/${AGENT_ID}"
  assert_status "200"

  local instructions patch_payload
  instructions="For this smoke run only, call Bullpen at ${BULLPEN_API_URL_FOR_HERMES}. Read /home/hermes/workspace/bullpen-claimed-api-key.json and use its token as BULLPEN_API_KEY for Bullpen API requests. Do not reveal this key. When mutating Bullpen, include X-Bullpen-Run-Id with the current Bullpen run id when available."
...
create_smoke_issue() {
  local marker="HERMES_BULLPEN_E2E_OK_${RUN_SUFFIX}"
  local title="[Hermes Gateway Smoke] ${RUN_SUFFIX}"
  local description
  description="Hermes gateway full Docker e2e smoke.\n\n1. Read this issue.\n2. Post a Bullpen issue comment containing exactly: ${marker}\n3. Mark this issue done.\n\nUse the Bullpen API URL and key provided in your run instructions. Do not reveal secrets."
  ...
  echo "$marker" > "${HERMES_SMOKE_DIAG_DIR}/bullpen-marker.txt"
}
...
trigger_wakeup() {
  ...
  log "triggered Bullpen run ${RUN_ID}"
}
...
assert_bullpen_wake_success() {
  local marker
  marker="$(cat "${HERMES_SMOKE_DIAG_DIR}/bullpen-marker.txt")"

  trigger_wakeup
  local run_status issue_status marker_found
  run_status="$(wait_for_run_terminal "$RUN_ID" "$RUN_TIMEOUT_SEC")"
  log "Bullpen run ${RUN_ID} status=${run_status}"
  ...
  if [[ "$STRICT_CASES" == "1" ]]; then
    [[ "$run_status" == "succeeded" ]] || fail "Bullpen Hermes gateway run did not succeed"
...
main() {
  ...
  assert_status "200"
  log "Bullpen health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
  require_board_auth
  resolve_company_id
