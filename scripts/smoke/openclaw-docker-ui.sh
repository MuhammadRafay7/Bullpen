...
BULLPEN_HOST_PORT="${BULLPEN_HOST_PORT:-3100}"
BULLPEN_HOST_FROM_CONTAINER="${BULLPEN_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.bullpen-openclaw.override.yml"
...
detect_bullpen_base_url() {
  ...
  for candidate in "$BULLPEN_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${BULLPEN_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${BULLPEN_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
bullpen_base_url="$(detect_bullpen_base_url || true)"
...
Bullpen URL for OpenClaw container:
EOF
  if [[ -n "$bullpen_base_url" ]]; then
    cat <<EOF
  $bullpen_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${BULLPEN_HOST_PORT}
  (Do not use http://127.0.0.1:${BULLPEN_HOST_PORT} inside the container.)
  If Bullpen rejects the host, run on host machine:
    pnpm bullpen allowed-hostname host.docker.internal
  Then restart Bullpen and re-run this script.
EOF
  fi
  cat <<EOF
