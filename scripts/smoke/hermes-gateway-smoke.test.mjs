...
BULLPEN_API_KEY="bullpen-secret"
BULLPEN_AUTH_HEADER="Bearer board-secret"
BULLPEN_COOKIE="session=board-cookie"
output="$(redact_text "gateway-secret claim-secret agent-secret bullpen-secret Bearer board-secret session=board-cookie")"
[[ "$output" != *"gateway-secret"* ]]
[[ "$output" != *"claim-secret"* ]]
[[ "$output" != *"agent-secret"* ]]
[[ "$output" != *"bullpen-secret"* ]]
[[ "$output" != *"board-secret"* ]]
[[ "$output" != *"board-cookie"* ]]
