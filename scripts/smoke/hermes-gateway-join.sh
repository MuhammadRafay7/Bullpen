...
Creates a Hermes gateway agent from an agent-only Bullpen invite, approves the
join request, claims the one-time Bullpen API key, and verifies the stored
...
Notes:
  HERMES_GATEWAY_API_BASE_URL is stored on the Bullpen adapter and must be
  reachable by the Bullpen server. HERMES_GATEWAY_PROBE_URL is only used by
  this operator shell to preflight /health, which is useful when Bullpen talks
  ...
  claimed Bullpen agent API key and is written chmod 600.
...
redact_text() {
  local text="$1"
  local secret
  for secret in "${HERMES_GATEWAY_API_KEY:-}" "${CLAIM_SECRET:-}" "${AGENT_API_KEY:-}" "${BULLPEN_AUTH_HEADER:-}" "${BULLPEN_COOKIE:-}" "${BULLPEN_API_KEY:-}"; do
    if [[ -n "$secret" ]]; then
...
Provide one of:
  BULLPEN_AUTH_HEADER="Bearer <board-token>"
  BULLPEN_COOKIE="<board-session-cookie>"
...
probe_hermes_gateway() {
  ...
  [[ -n "$HERMES_GATEWAY_API_KEY" ]] || fail "HERMES_GATEWAY_API_KEY or API_SERVER_KEY is required before any Bullpen state is mutated"
  ...
  if [[ "$code" != "200" ]]; then
    fail "Hermes gateway health probe failed before mutating Bullpen state: ${health_url} returned HTTP ${code}. Start Hermes with API_SERVER_ENABLED=true API_SERVER_KEY=<key> hermes gateway run --replace --accept-hooks, or set HERMES_GATEWAY_API_BASE_URL/HERMES_GATEWAY_API_KEY."
  fi
}

log "checking Bullpen health"
...
JOIN_PAYLOAD="$(jq -nc \
  ...
  --arg bullpenApiUrl "$BULLPEN_API_URL_FOR_HERMES" \
  ...
  | if $bullpenApiUrl != "" then .agentDefaultsPayload.bullpenApiUrl = $bullpenApiUrl else . end
...
if [[ -n "$BULLPEN_API_URL_FOR_HERMES" ]]; then
  STORED_BULLPEN_API_URL="$(jq -r '.adapterConfig.bullpenApiUrl // empty' <<<"$RESPONSE_BODY")"
  [[ "$STORED_BULLPEN_API_URL" == "$BULLPEN_API_URL_FOR_HERMES" || "$(strip_trailing_slash "$STORED_BULLPEN_API_URL")" == "$(strip_trailing_slash "$BULLPEN_API_URL_FOR_HERMES")" ]] \
    || fail "stored bullpenApiUrl mismatch"
fi
