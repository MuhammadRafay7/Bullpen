...
: "${BULLPEN_API_URL:?Set BULLPEN_API_URL for the target dev instance.}"
: "${BULLPEN_API_KEY:?Set BULLPEN_API_KEY for the target dev instance.}"
: "${BULLPEN_COMPANY_ID:?Set BULLPEN_COMPANY_ID for the target dev company.}"

read -r -a PC_CMD <<< "${BULLPEN_CMD:-pnpm --silent bullpen}"
...
pc_json() {
  "${PC_CMD[@]}" "$@" --json -C "$BULLPEN_COMPANY_ID"
}
