...
IMAGE_NAME="${IMAGE_NAME:-bullpen-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
BULLPEN_VERSION="${BULLPEN_VERSION:-latest}"
...
BULLPEN_DEPLOYMENT_MODE="${BULLPEN_DEPLOYMENT_MODE:-authenticated}"
BULLPEN_DEPLOYMENT_EXPOSURE="${BULLPEN_DEPLOYMENT_EXPOSURE:-private}"
BULLPEN_PUBLIC_URL="${BULLPEN_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@bullpen.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-bullpen-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_BULLPEN_VERSION=%q\n' "$BULLPEN_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e BULLPEN_DEPLOYMENT_MODE="$BULLPEN_DEPLOYMENT_MODE" \
      -e BULLPEN_DEPLOYMENT_EXPOSURE="$BULLPEN_DEPLOYMENT_EXPOSURE" \
      -e BULLPEN_PUBLIC_URL="$BULLPEN_PUBLIC_URL" \
      -e BULLPEN_HOME="/bullpen" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "bullpen@${BULLPEN_VERSION}" auth bootstrap-ceo --data-dir "$BULLPEN_HOME" --base-url "$BULLPEN_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$BULLPEN_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$BULLPEN_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$BULLPEN_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$BULLPEN_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$BULLPEN_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$BULLPEN_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg BULLPEN_VERSION="$BULLPEN_VERSION" \
...
echo "    Public URL: $BULLPEN_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $BULLPEN_DEPLOYMENT_MODE/$BULLPEN_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bullpen-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$BULLPEN_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $BULLPEN_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$BULLPEN_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $BULLPEN_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
