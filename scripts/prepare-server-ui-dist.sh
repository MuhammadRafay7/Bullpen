...
# This keeps @bullpen/server publish artifacts self-contained for static UI serving.
# When BULLPEN_RELEASE_REUSE_UI_DIST=1 and ui/dist already exists, reuse that
...
case "${BULLPEN_RELEASE_REUSE_UI_DIST:-}" in
  1|true|TRUE|yes|YES)
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @bullpen/ui dist output"
else
  echo "  -> Building @bullpen/ui..."
  pnpm --dir "$REPO_ROOT" --filter @bullpen/ui build
fi
