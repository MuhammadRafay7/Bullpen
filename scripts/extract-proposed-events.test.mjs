...
function withFixtureRepo(source, callback) {
  const repoRoot = mkdtempSync(join(tmpdir(), "bullpen-proposed-events-"));
...
test("provenance paths are repo-relative and reject dev-host path shapes", () => {
  ...
  const repoRoot = mkdtempSync(join(tmpdir(), "bullpen-provenance-root-"));
  try {
...
function diagnosticsFor(sourceText) {
  const repoRoot = mkdtempSync(join(tmpdir(), "bullpen-ts2578-"));
