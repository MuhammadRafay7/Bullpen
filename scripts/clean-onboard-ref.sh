...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Bullpen data dir to use
  BULLPEN_HOST=127.0.0.1    Host passed to the onboarded server
  BULLPEN_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/bullpen-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/bullpen-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export BULLPEN_HOST="${BULLPEN_HOST:-127.0.0.1}"
export BULLPEN_PORT="${BULLPEN_PORT:-3100}"
export BULLPEN_OPEN_ON_LISTEN="${BULLPEN_OPEN_ON_LISTEN:-false}"
...
echo "BULLPEN_HOST: $BULLPEN_HOST"
echo "BULLPEN_PORT: $BULLPEN_PORT"
...
env \
  ...
  HOST="$BULLPEN_HOST" \
  PORT="$BULLPEN_PORT" \
  BULLPEN_OPEN_ON_LISTEN="$BULLPEN_OPEN_ON_LISTEN" \
  pnpm bullpen onboard --yes --data-dir "$PC_DATA"
