...
const DEFAULT_QUERY = "\"Co-Authored-By: Bullpen <noreply@paperclip.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/bullpen-commit-metrics-cache.json");
...
const BULLPEN_EMAIL = "noreply@paperclip.ing";
const BULLPEN_NAME = "bullpen";
...
function printHelp() {
  console.log(`Usage: tsx scripts/bullpen-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === BULLPEN_EMAIL) || (displayName && displayName.toLowerCase() === BULLPEN_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Bullpen commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "bullpen-commit-metrics",
