...
-- Keeps the oldest row per (company_id) where driver='sandbox' AND metadata.managedByBullpen is true.
-- Scoped to Bullpen-managed rows only: tenant-created sandbox envs (without the marker) are untouched.
DO $$
DECLARE
...
BEGIN
  WITH ranked AS (
    ...
    WHERE driver = 'sandbox'
      AND (metadata ->> 'managedByBullpen')::boolean = true
  ),
  deleted AS (
