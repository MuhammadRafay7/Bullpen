...
import {
  ...
  resolveBullpenEnvPathForConfig,
...
export type ResolvedDatabaseTarget =
  | {
      mode: "postgres";
      connectionString: string;
      source: "DATABASE_URL" | "bullpen-env" | "config.database.connectionString";
      configPath: string;
      envPath: string;
    }
  | {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".bullpen", CONFIG_BASENAME);
...
function resolveBullpenConfigPath(): string {
  if (process.env.BULLPEN_CONFIG?.trim()) {
    return path.resolve(process.env.BULLPEN_CONFIG.trim());
  }
  return findConfigFileFromAncestors(process.cwd()) ?? resolveBullpenConfigPathForInstance();
}

function resolveBullpenEnvPath(configPath: string): string {
  return resolveBullpenEnvPathForConfig(configPath);
}
...
export function resolveDatabaseTarget(): ResolvedDatabaseTarget {
  ...
  const envPath = resolveBullpenEnvPath(configPath);
