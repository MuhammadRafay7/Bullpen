...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://bullpen:bullpen@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "bullpen");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://bullpen:bullpen@127.0.0.1:${preferredPort}/bullpen`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://bullpen:bullpen@127.0.0.1:${port}/bullpen`,
  ...
  const adminConnectionString = `postgres://bullpen:bullpen@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "bullpen");

  return {
    connectionString: `postgres://bullpen:bullpen@127.0.0.1:${selectedPort}/bullpen`,
