...
async function getIssueWorkspaceRuntime(client: BullpenApiClient, issueId: string) {
...
export function createToolDefinitions(client: BullpenApiClient): ToolDefinition[] {
  return [
    makeTool(
      "bullpenMe",
      "Get the current authenticated Bullpen actor details",
      z.object({}),
    ...
    makeTool(
      "bullpenRequestCheckboxConfirmation",
    ...
    makeTool(
      "bullpenApiRequest",
      "Make a JSON request to an existing Bullpen /api endpoint for unsupported operations",
      apiRequestSchema,
