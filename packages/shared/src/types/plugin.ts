 ...
 *
 * The Bullpen extension keywords below are recognised by the Bullpen UI
 * but are otherwise ignored by standard JSON Schema validators.
 */
export type JsonSchema = {
  /**
   * When true, the Bullpen config UI hides this property behind an
   * "Advanced options" disclosure. Defaults to false (always visible).
   */
  "x-bullpen-advanced"?: boolean;
  /**
   ...
   * `x-bullpen-advanced` is not true.
   */
  "x-bullpen-group"?: string;
  [key: string]: unknown;
};
...
// ---------------------------------------------------------------------------
// Manifest sub-types — nested declarations within BullpenPluginManifestV1
// ---------------------------------------------------------------------------
...
export interface PluginEnvironmentDriverDeclaration {
  ...
  /**
   * How Bullpen should apply a captured template ref back into this provider's
...
/**
 * Declares a normal Bullpen agent that a plugin can provision and later
...
export interface PluginManagedAgentDeclaration {
  ...
  /** Suggested Bullpen runtime configuration. */
...
/**
 * Declares a normal Bullpen project that a plugin can provision and later
 * resolve by stable key within each company.
 */
...
/**
 * Lower-bound semver requirement for the Bullpen host.
 *
...
export interface BullpenPluginManifestV1 {
   ...
   */
  minimumBullpenVersion?: PluginMinimumHostVersion;
