...
Example Bullpen plugin that demonstrates:
...
```bash
pnpm --filter @bullpen/plugin-file-browser-example build
pnpm bullpen plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm bullpen plugin uninstall bullpen-file-browser-example --force
```
...
- Optional: use `bullpen-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
