...
type BullpenSprite = (typeof BULLPEN_SPRITES)[number];
...
function spriteSize(sprite: BullpenSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BULLPEN_SPRITES[(baseCol + baseRow) % BULLPEN_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BULLPEN_SPRITES[Math.floor(Math.random() * BULLPEN_SPRITES.length)]!;
