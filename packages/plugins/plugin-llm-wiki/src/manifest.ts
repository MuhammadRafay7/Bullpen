...
export const PLUGIN_ID = "bullpen.plugin-llm-wiki";
...
export const BULLPEN_DISTILL_SKILL_KEY = "bullpen-distill";
...
export const DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_BULLPEN_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_BULLPEN_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/bullpen-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Bullpen issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Bullpen-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Bullpen ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Bullpen issue with the exact files and evidence.
...
const manifest: BullpenPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Bullpen project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: BULLPEN_DISTILL_SKILL_KEY,
      displayName: "Bullpen Distill",
      slug: BULLPEN_DISTILL_SKILL_KEY,
      description: "Turn Bullpen cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(BULLPEN_DISTILL_SKILL_KEY)
    },
    {
