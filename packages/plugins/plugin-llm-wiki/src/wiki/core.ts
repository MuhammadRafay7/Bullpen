...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@bullpen/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@bullpen/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_BULLPEN_ROUTINE_RUN_CHARS,
  DEFAULT_BULLPEN_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_BULLPEN_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_BULLPEN_DISTILLATION_FAN_OUT = 25;
const MAX_BULLPEN_PROFILE_SELECTED_PROJECTS = 25;
const MAX_BULLPEN_PROFILE_ROOT_ISSUES = 25;
...
export type BullpenDistillationScope = "company" | "project" | "root_issue";
export type BullpenDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type BullpenIngestionSourceScope =
...
export type BullpenIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: BullpenIngestionSourceScope[];
...
export type BullpenIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type BullpenIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: BullpenIngestionProfileV1;
  effectiveState: BullpenIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type BullpenIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type BullpenIngestionPolicyDecision =
...
type BullpenIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type BullpenSourceBundle = {
  markdown: string;
  sourceRefs: BullpenSourceRef[];
  sourceHash: string;
...
type BullpenDistillationRunInput = BullpenSourceBundleInput;

type BullpenDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type BullpenDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: BullpenDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type BullpenProjectPageDistillationInput = BullpenSourceBundleInput & {
  autoApply?: boolean;
...
type BullpenDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type BullpenDistillationPatch = {
  pagePath: string;
  operationType: BullpenDistillationPatchOperation;
...
async function requireBullpenIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: BullpenIngestionPolicyPurpose,
  ...
  const decision = evaluateBullpenProfilePolicy({
    space,
    profile,
    purpose,
...
function assertBullpenSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Bullpen source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Bullpen ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_BULLPEN_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type BullpenDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<BullpenSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Bullpen source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveBullpenDistillationLimits(
  ctx: PluginContext,
  input: Pick<BullpenSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<BullpenDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get(input.companyId) as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxBullpenIssueSourceCharacters, DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxBullpenCursorWindowCharacters,
    DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxBullpenRoutineRunCharacters,
    DEFAULT_MAX_BULLPEN_ROUTINE_RUN_CHARS,
  );
  ...
  return {
    ...
    costCentsPerThousandSourceCharacters: normalizeCostRate(config.bullpenCostCentsPerThousandSourceCharacters),
  };
}

async function resolveBullpenDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<BullpenSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<BullpenDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveBullpenDistillationLimits(ctx, input),
...
function defaultBullpenIngestionProfile(input: {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS,
...
function normalizeBullpenIngestionSourceScope(value: unknown): BullpenIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_BULLPEN_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_BULLPEN_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_BULLPEN_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeBullpenIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): BullpenIngestionProfileV1 {
  const fallback = defaultBullpenIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeBullpenIngestionSourceScope).filter((scope): scope is BullpenIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_BULLPEN_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_BULLPEN_ISSUE_SOURCE_CHARS),
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: WikiSpace): Promise<BullpenIngestionProfileV1> {
  const legacySettings = space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, companyId) : null;
  return normalizeBullpenIngestionProfile(space.settings.bullpenIngestion, { space, legacySettings });
}
...
function evaluateBullpenProfilePolicy(input: {
  space: WikiSpace;
  profile?: BullpenIngestionProfileV1 | null;
  purpose: BullpenIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): BullpenIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Bullpen ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Bullpen ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Bullpen sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Bullpen ingestion policy denied ${purpose}: Bullpen ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Bullpen ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getBullpenIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<BullpenIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateBullpenProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countBullpenHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countBullpenProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: BullpenIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countBullpenHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: BullpenIngestionSourceScope): string[] {
...
async function countBullpenProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: BullpenIngestionProfileV1;
...
async function validateBullpenIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: BullpenIngestionProfileV1;
}) {
  const policy = evaluateBullpenProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Bullpen ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_BULLPEN_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Bullpen ingestion profile sources exceed the hard cap of ${MAX_BULLPEN_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_BULLPEN_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_BULLPEN_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_BULLPEN_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_BULLPEN_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateBullpenIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeBullpenIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateBullpenIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  await updateSpace(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceSlug: space.slug,
    settings: { bullpenIngestion: profile },
  });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Bullpen ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.bullpen_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getBullpenIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listBullpenIngestionCandidates(ctx: PluginContext, input: BullpenIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
  ...
  await requireBullpenIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_BULLPEN_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`Bullpen ingestion profile sources exceed the hard cap of ${MAX_BULLPEN_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeBullpenIngestionProfile(
    {
      ...defaultBullpenIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultBullpenIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".bullpen/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "bullpen_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "bullpen_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "bullpen_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "bullpen_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "bullpen_page_bindings");
}
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  const bullpenDerived = input.operationType === "distill" || input.operationType === "backfill";
  return [
    ...
    bullpenDerived
      ? "- Bullpen-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Bullpen-derived pages into a non-default space."
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
function bullpenDistillationScope(input: { projectId?: string | null; rootIssueId?: string | null }): BullpenDistillationScope {
  if (input.rootIssueId) return "root_issue";
  if (input.projectId) return "project";
  return "company";
}

function bullpenCursorScopeMetadata(input: { projectId?: string | null; rootIssueId?: string | null }) {
  const sourceScope = bullpenDistillationScope(input);
...
async function upsertBullpenDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  const scope = bullpenCursorScopeMetadata(input);
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS bullpen_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'bullpen_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(bullpen_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, bullpen_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = bullpen_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = bullpen_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'bullpen_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_BULLPEN_DISTILLATION_FAN_OUT) {
    throw new Error(`Bullpen ingestion fan-out exceeds the hard cap of ${MAX_BULLPEN_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
  ...
  for (const project of activeProjects) {
    ...
    const cursorId = await upsertBullpenDistillationCursor(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceId: space.id,
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: BullpenSourceRef[];
  ref: BullpenSourceRef;
...
function sourceRefUpdatedAt(ref: BullpenSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<BullpenSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listBullpenBundleIssues(ctx: PluginContext, input: BullpenSourceBundleInput): Promise<Issue[]> {
...
export async function assembleBullpenSourceBundle(ctx: PluginContext, input: BullpenSourceBundleInput): Promise<BullpenSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
  assertBullpenSourceScopePayload(input);
  const space = await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const limits = await resolveBullpenDistillationLimitsForSpace(ctx, { ...input, space });
  ...
  const issues = await listBullpenBundleIssues(ctx, input);
  const scope = bullpenCursorScopeMetadata(input);
  const sourceRefs: BullpenSourceRef[] = [];
  const warnings: string[] = [];
  const lines = [
    `# Bullpen source bundle`,
    "",
    "## Bundle Metadata",
    "",
...
export async function createBullpenDistillationRun(ctx: PluginContext, input: BullpenDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertBullpenSourceScopePayload(input);
  const space = await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = bullpenCursorScopeMetadata(input);
  ...
  const bundle = await assembleBullpenSourceBundle(ctx, input);
...
export async function recordBullpenDistillationOutcome(ctx: PluginContext, input: BullpenDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createBullpenDistillationWorkItem(ctx: PluginContext, input: BullpenDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertBullpenSourceScopePayload(input);
  const space = await requireBullpenIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS bullpen_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = bullpen_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: BullpenSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Bullpen source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: BullpenSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): BullpenSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function projectPageSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
  return slugify(input.project?.name ?? input.rootIssue?.title ?? "bullpen-project");
}
...
function hasDurableSignal(bundle: BullpenSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: BullpenSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "Bullpen Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Bullpen work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Bullpen issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Bullpen before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: BullpenSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Bullpen Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: BullpenSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "Bullpen Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] bullpen-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS bullpen_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = bullpen_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.BULLPEN_DEPLOYMENT_MODE;
  const rawExposure = process.env.BULLPEN_DEPLOYMENT_EXPOSURE;
...
export async function distillBullpenProjectPage(ctx: PluginContext, input: BullpenProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createBullpenDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordBullpenDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal Bullpen source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Bullpen source window."],
      patches: [] as BullpenDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordBullpenDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Bullpen source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Bullpen source hash."],
      patches: [] as BullpenDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Bullpen Project";
  ...
  const patches: BullpenDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Bullpen distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForBullpenEvent(input: {
  ...
  return assertRawPath(`raw/bullpen/${input.sourceKind}/${eventDate}-${slugify(identifier)}-${slugify(input.label)}-${contentHash(input.contents).slice(0, 8)}.md`);
}

function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Bullpen issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Bullpen comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Bullpen document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordBullpenCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<BullpenEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertBullpenDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function bullpenProfileIncludesIssue(ctx: PluginContext, input: {
  companyId: string;
  issue: Issue;
  profile: BullpenIngestionProfileV1;
}): Promise<boolean> {
...
async function routeBullpenCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<BullpenEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<BullpenEventIngestResult, { status: "recorded" }>> = [];
  let eligibleProfileCount = 0;
  for (const space of spaces) {
    const profile = await profileForSpace(ctx, input.companyId, space);
    if (!profile.enabled) continue;
    const policy = evaluateBullpenProfilePolicy({ space, profile, purpose: "event_routing", requireEnabledProfile: true });
    if (!policy.allowed) continue;
    if (!profile.sourceKinds[input.sourceKind]) continue;
    if (!(await bullpenProfileIncludesIssue(ctx, { companyId: input.companyId, issue: input.issue, profile }))) continue;
    eligibleProfileCount += 1;
    if (eligibleProfileCount > MAX_BULLPEN_DISTILLATION_FAN_OUT) {
      throw new Error(`Bullpen ingestion fan-out exceeds the hard cap of ${MAX_BULLPEN_DISTILLATION_FAN_OUT} enabled profiles.`);
    }
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.wikiId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
    recorded.push(await recordBullpenCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleBullpenEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<BullpenEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeBullpenCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,
