...
const EXPANDED_STORAGE_KEY = `bullpen.plugin-llm-wiki:route-sidebar-expanded:v2:${COMPANY_ID}`;
...
describe("WikiRouteSidebar", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as BridgeGlobal).__bullpenPluginBridge__ = {
      sdkUi: {
  ...
  afterEach(() => {
    ...
    container.remove();
    window.localStorage.clear();
    delete (globalThis as BridgeGlobal).__bullpenPluginBridge__;
  });
  ...
  it("keeps sidebar tree selection scoped to sidebar navigation", () => {
    ...
    expect(navigatedTo).toEqual({
      ...
      options: { state: { bullpenWikiSidebarTreePath: "wiki/concepts/sidebar-navigation.md" } },
    });
    ...
    hostLocation = {
      pathname: "/PAP/wiki/page/wiki/entities/bullpen.md",
      search: "",
      hash: "",
    };
    act(() => {
...
describe("WikiPage", () => {
  ...
  it("renders structured Bullpen source refs as text", () => {
    act(() => {
