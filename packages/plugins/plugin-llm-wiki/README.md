...
here rather than in Bullpen core.
...
- opt-in company-scoped Bullpen event ingestion controls for issues, comments, and documents; event ingestion is disabled by default and routes captured raw provenance into the default space only
- manual Bullpen project/root issue distillation and bounded backfill actions with explicit work items, operation issues, source caps, and estimated cost recording
- Bullpen-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default wiki space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space Bullpen ingestion profiles ship
- Bullpen-derived distillation maintains `wiki/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `wiki/projects/<slug>/index.md` knowledge pages
...
Bullpen-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
The accepted policy lives in [doc/plans/2026-05-06-llm-wiki-bullpen-asset-security-gate.md](../../../doc/plans/2026-05-06-llm-wiki-bullpen-asset-security-gate.md).
...
From the Bullpen repo root:

```bash
pnpm --filter @bullpen/plugin-llm-wiki typecheck
pnpm --filter @bullpen/plugin-llm-wiki test
pnpm --filter @bullpen/plugin-llm-wiki build
```

## Alpha Verification

Run these commands from the Bullpen repo root before handing off alpha plugin
changes:
...
- disabled and enabled Bullpen event ingestion paths
...
- Host-level plugin install and live agent invocation still need Bullpen
  server/runtime smoke coverage when preparing a release candidate.



## Install Into Bullpen

```bash
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  -H "Content-Type: application/json" \
  -d '{"packageName":"/Users/dotta/bullpen/.bullpen/worktrees/PAP-3179-design-a-llm-wiki-plugin/packages/plugins/plugin-llm-wiki","isLocalPath":true}'
```

## Build Options

- `pnpm build` uses esbuild presets from `@bullpen/plugin-sdk/bundlers`.
...
The package-local `dist/` directory is ignored by git, but local Bullpen
...
restart the Bullpen dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses Bullpen's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and Bullpen project overviews, standups,
