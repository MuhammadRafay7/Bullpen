 ...
 *
 * These components are exported from `@bullpen/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@bullpen/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function BullpenSdkUiComponent(props: TProps) {
...
/**
 * Renders Bullpen's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders Bullpen's native issue list component for company-scoped plugin
...
/**
 * Renders Bullpen's native managed routines list for plugin settings pages.
 */
