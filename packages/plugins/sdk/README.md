...
Official TypeScript SDK for Bullpen plugin authors.

- **Worker SDK:** `@bullpen/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@bullpen/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@bullpen/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@bullpen/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@bullpen/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@bullpen/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@bullpen/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@bullpen/plugin-sdk/ui/hooks` | Hooks only |
| `@bullpen/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@bullpen/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@bullpen/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@bullpen/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@bullpen/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@bullpen/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Bullpen app. They can call ordinary Bullpen HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Bullpen instance at runtime.
...
- The host ships a small shared React component kit through `@bullpen/plugin-sdk/ui`. Use it for native Bullpen controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@bullpen/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@bullpen/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Bullpen information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@bullpen/plugin-sdk`.
...
metadata. Slow or failing plugins are isolated: Bullpen logs the failure and
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:bullpen.missions:feature",
...
await ctx.issues.createComment(issueId, replyText, companyId, {
  actorUserId: verifiedSlackUser.bullpenUserId,
});
```
...
```tsx
import { usePluginData, usePluginAction } from "@bullpen/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@bullpen/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@bullpen/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@bullpen/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@bullpen/plugin-sdk/ui";
...
Routes Bullpen-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@bullpen/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for Bullpen-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@bullpen/plugin-sdk/ui";
...
The host provides selected shared UI components through `@bullpen/plugin-sdk/ui`.
...
used by Bullpen issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@bullpen/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/bullpen|Bullpen]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@bullpen/plugin-sdk/ui";
...
or configure work against Bullpen entities. Both are controlled components and
load their options from the host for the provided company.

```tsx
import { AssigneePicker, ProjectPicker } from "@bullpen/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@bullpen/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@bullpen/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@bullpen/plugin-sdk/bundlers";
...
```bash
bullpen-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@bullpen/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__bullpen__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__bullpen__/events` streams `reload` SSE events on UI build changes
