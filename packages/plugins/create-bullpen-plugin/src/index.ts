...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".bullpen-sdk");
...
/**
 * Generate a complete Bullpen plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Bullpen plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "bullpen-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["bullpen", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@bullpen/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@bullpen/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@bullpen/plugin-sdk/ui";
...
} from "@bullpen/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@bullpen/plugin-sdk/ui";
...
When this package is installed from a local path, Bullpen watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@bullpen/plugin-sdk\` and \`@bullpen/shared\` from a local Bullpen checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.bullpen-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into Bullpen

\`\`\`bash
bullpen plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
