...
export function parseNovitaDriverConfig(raw: Record<string, unknown>): NovitaDriverConfig {
  return {
    ...
    requestedCwd: parseOptionalString(raw.requestedCwd) ?? "/home/user/bullpen-workspace",
...
function buildStdinPath(): string {
  return `/tmp/.bullpen-stdin-${randomUUID()}`;
}
...
async function createSandbox(params: PluginEnvironmentAcquireLeaseParams | PluginEnvironmentProbeParams, config: NovitaDriverConfig) {
  const metadata = {
    "bullpen-provider": "novita",
    "bullpen-company-id": params.companyId,
    "bullpen-environment-id": params.environmentId,
    ...(params.issueId ? { "bullpen-issue-id": params.issueId } : {}),
    ...("runId" in params ? { "bullpen-run-id": params.runId } : {}),
  };
  const opts = {
    ...sandboxOpts(config),
    metadata,
