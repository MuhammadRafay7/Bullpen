...
const DEFAULT_WORKDIR = "/workspace/bullpen";
...
function resolveAuth(config: ModalDriverConfig): { tokenId: string; tokenSecret: string } | null {
  ...
  // environment config, which Bullpen stores as company secrets.
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf bullpen-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "bullpen-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/bullpen-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
