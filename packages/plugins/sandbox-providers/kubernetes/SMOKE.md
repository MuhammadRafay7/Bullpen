# Manual smoke test — `@bullpen/plugin-kubernetes`

Manual sanity check that the plugin works end-to-end against a real
bullpen-server instance and a real Kubernetes cluster (kind for local
...
- A running kind cluster:
  ```bash
  kind create cluster --name bullpen
  ```
- `kubectl --context kind-bullpen get nodes` returns a node in `Ready` state.
...
### 2. Start bullpen-server in dev mode

In a separate terminal:

```bash
cd /path/to/bullpen
export BULLPEN_HOME=/tmp/bullpen-smoke
export BULLPEN_INSTANCE_ID=smoke
export BULLPEN_DEPLOYMENT_MODE=local_trusted
pnpm --filter @bullpen/server dev
```
...
```bash
pnpm bullpen plugin install \
  --local /path/to/bullpen/packages/plugins/sandbox-providers/kubernetes \
  --api-base http://127.0.0.1:3100
```

Expected: `✓ Installed bullpen.kubernetes-sandbox-provider v0.1.0 (ready)`.
...
(`bullpen-smoke`). On first probe the namespace may not yet exist —
...
1. `ensureTenant` — provision the `bullpen-smoke` namespace, SA, Role,
...
```bash
kubectl --context kind-bullpen get namespace bullpen-smoke
kubectl --context kind-bullpen get all,networkpolicy,resourcequota,limitrange,sa,role,rolebinding -n bullpen-smoke
```

Expected:

- Namespace `bullpen-smoke` exists with PSS labels
  (`pod-security.kubernetes.io/enforce=restricted`)
- ServiceAccount `bullpen-tenant-sa`
- Role `bullpen-tenant-role`, RoleBinding `bullpen-tenant-rb`
- ResourceQuota `bullpen-quota`, LimitRange `bullpen-limits`
- NetworkPolicies `bullpen-deny-all` + `bullpen-egress-allow`
...
```bash
kubectl --context kind-bullpen delete namespace bullpen-smoke
kill %1  # bullpen-server
```
...
- Output of `kubectl get all -n bullpen-smoke` after step 6
