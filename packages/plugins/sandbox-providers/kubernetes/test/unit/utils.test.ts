...
import { deriveCompanySlug, deriveNamespaceName, newRunUlidDns, bullpenLabels } from "../../src/utils.js";
...
describe("deriveNamespaceName", () => {
  it("concatenates prefix and slug", () => {
    expect(deriveNamespaceName("bullpen-", "acme-co")).toBe("bullpen-acme-co");
  });
});
...
describe("bullpenLabels", () => {
  it("returns canonical label map", () => {
    const labels = bullpenLabels({ runId: "r1", agentId: "a1", companyId: "c1", adapterType: "claude_local" });
    expect(labels["bullpen.io/run-id"]).toBe("r1");
    expect(labels["bullpen.io/agent-id"]).toBe("a1");
    expect(labels["bullpen.io/company-id"]).toBe("c1");
    expect(labels["bullpen.io/adapter"]).toBe("claude_local");
    expect(labels["bullpen.io/managed-by"]).toBe("bullpen-k8s-plugin");
  });
});
