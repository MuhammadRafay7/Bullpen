 ...
 *        docker tag alpine:3.20 localhost/bullpen-agent:latest
 *        kind load docker-image localhost/bullpen-agent:latest --name bullpen
 ...
 *      And a tini-bearing image pre-loaded (e.g. the same localhost/bullpen-agent:latest
 ...
 * ("bullpen-"), resolving to "bullpen-spike-e2e".
 */
...
const NAMESPACE = "bullpen-spike-e2e";
...
describe("plugin-kubernetes end-to-end", () => {
  ...
  it.runIf(process.env.RUN_K8S_INTEGRATION_TESTS === "1")(
    ...
    async () => {
      ...
      expect(all).toContain("serviceaccount/bullpen-tenant-sa");
      expect(all).toContain("role.rbac.authorization.k8s.io/bullpen-tenant-role");
      expect(all).toContain("rolebinding.rbac.authorization.k8s.io/bullpen-tenant-rb");
      expect(all).toContain("resourcequota/bullpen-quota");
      expect(all).toContain("limitrange/bullpen-limits");
      expect(all).toContain("networkpolicy.networking.k8s.io/bullpen-deny-all");
      expect(all).toContain("networkpolicy.networking.k8s.io/bullpen-egress-allow");
