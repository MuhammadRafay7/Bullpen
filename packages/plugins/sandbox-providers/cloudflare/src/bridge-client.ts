...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Bullpen-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Bullpen-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Bullpen-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/bullpen-sandbox/v1";
