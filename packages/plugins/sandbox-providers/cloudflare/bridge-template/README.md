...
This Worker is the operator-facing bridge used by `@bullpen/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/bullpen-sandbox/v1` and translates Bullpen lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Bullpen does not inherit wedged background processes
...
After deploy, configure Bullpen with:
