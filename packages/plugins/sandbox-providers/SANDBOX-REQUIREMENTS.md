...
must meet this contract. The Bullpen runtime does not build the environment at
...
Bullpen probes each CLI before launch. Bullpen uses the same detection
...
Bullpen probes each CLI with `command -v <cmd>`. Bullpen fails loudly when
...
- The Bullpen runtime never modifies the login profile. The runtime never
  writes a profile file. The runtime never writes an rc file.
- The Bullpen runtime never sources `nvm` on the exec path.
