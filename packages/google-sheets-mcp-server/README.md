...
First-party MCP server for Google Sheets API v4. It can run as a Bullpen
`local_stdio` gallery connection or as a local Streamable HTTP server for
Bullpen's `remote_http` connect-by-link flow.
...
```sh
bullpen-google-sheets-mcp-server \
...
## Bullpen `local_stdio` Test Path

Use the Google Sheets gallery app when you want Bullpen to supervise the
server as a stdio MCP process:

1. Configure Bullpen's Google Sheets service account environment so the
...
2. Share every spreadsheet you want to test with the service account's
   `client_email`.
3. In Bullpen, open the tool app gallery, choose **Google Sheets**, and paste
   one or more Google Sheets links.
4. Save the app connection. Bullpen creates a `local_stdio` connection using
   the `bullpen.google-sheets` template and passes the selected spreadsheet
...
bullpen-google-sheets-mcp-http-server
```
...
Then in Bullpen, choose the remote HTTP or "connect with a link" path, paste
...
`GOOGLE_SHEETS_SPREADSHEET_IDS`), not a per-connection Bullpen gallery wizard
