import { asBoolean, asString, asStringArray } from "@bullpen/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Bullpen will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
