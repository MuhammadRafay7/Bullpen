...
This document describes how `@bullpen/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Bullpen logs/transcript parsing
...
Like every Bullpen adapter, this one must treat the local execution-workspace
...
the OpenClaw side, use the round-trip helpers in `@bullpen/adapter-utils`
