...
Use when:
- You want Bullpen to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Bullpen server.
...
- bullpenApiUrl (string, optional): absolute Bullpen base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/bullpen-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default bullpen)

Wake payload notes:
- Bullpen wake context is embedded into the generated message text
- No top-level bullpen field is sent; the gateway agent schema rejects unknown root params
