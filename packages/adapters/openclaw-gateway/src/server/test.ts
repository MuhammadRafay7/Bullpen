...
import { asString, parseObject } from "@bullpen/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Bullpen server host.",
        });
      }
    } catch (err) {
      checks.push({
