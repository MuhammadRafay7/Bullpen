...
## Scope
This plan is now **gateway-only**. Bullpen supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Bullpen skill.
...
```json
{
  "bullpenApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Bullpen task.
...
```bash
pnpm --filter @bullpen/server test:run -- openclaw-gateway
pnpm --filter @bullpen/server typecheck
pnpm --filter @bullpen/ui typecheck
pnpm --filter bullpen typecheck
```
