...
type SkillEntry = BullpenSkillEntry;
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("bullpen-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureBullpenSkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[bullpen] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
