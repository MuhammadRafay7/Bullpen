...
describe("pi remote execution", () => {
  ...
  it("prepares the workspace, syncs Pi skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-"));
    ...
    expect(String(result.sessionId)).toContain(`${managedRemoteWorkspace}/.bullpen-runtime/pi/sessions/`);
    expect(prepareWorkspaceForSshExecution).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.bullpen-runtime/pi/skills`,
      followSymlinks: true,
    }));
    expect(runSshCommand).toHaveBeenCalledWith(
      expect.anything(),
      expect.stringContaining(".bullpen-runtime/pi/sessions"),
      expect.anything(),
    );
    ...
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.bullpen-runtime/pi/skills`);
  ...
  it("ships the managed Pi agent config and repoints PI_CODING_AGENT_DIR when BULLPEN_PI_PROVIDERS is set", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-providers-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    const managedRemoteWorkspace = "/remote/workspace/.bullpen-runtime/runs/run-providers/workspace";
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      config: {
        ...
        env: {
          BULLPEN_PI_PROVIDERS: JSON.stringify(providers),
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.bullpen-runtime/pi/agentConfig`,
    }));
    const call = runChildProcess.mock.calls[0] as unknown as
      | [string, string, string[], { env: Record<string, string> }]
      | undefined;
    expect(call?.[3].env.PI_CODING_AGENT_DIR).toBe(
      `${managedRemoteWorkspace}/.bullpen-runtime/pi/agentConfig`,
    );
  ...
  it("resumes saved Pi sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    ...
    await execute({
      ...
      runtime: {
        sessionId: `${managedRemoteWorkspace}/.bullpen-runtime/pi/sessions/session-123.jsonl`,
        sessionParams: {
          sessionId: `${managedRemoteWorkspace}/.bullpen-runtime/pi/sessions/session-123.jsonl`,
          cwd: managedRemoteWorkspace,
          remoteExecution: {
    ...
    expect(call?.[2]).toContain("--session");
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.bullpen-runtime/pi/sessions/session-123.jsonl`);
  });

  it("starts a fresh remote Pi session when the saved session header cwd points at a different workspace", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-stale-session-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      runtime: {
        sessionId: "/remote/workspace/.bullpen-runtime/pi/sessions/session-123.jsonl",
        sessionParams: {
          sessionId: "/remote/workspace/.bullpen-runtime/pi/sessions/session-123.jsonl",
    ...
    const managedRemoteWorkspaceFresh = "/remote/workspace/.bullpen-runtime/runs/run-ssh-stale-session/workspace";
    const call = runChildProcess.mock.calls[0] as unknown as [string, string, string[]] | undefined;
    const sessionIndex = call?.[2].indexOf("--session") ?? -1;
    expect(sessionIndex).toBeGreaterThanOrEqual(0);
    const usedSession = sessionIndex >= 0 ? call?.[2][sessionIndex + 1] : null;
    expect(usedSession).toContain(`${managedRemoteWorkspaceFresh}/.bullpen-runtime/pi/sessions/`);
    expect(usedSession).not.toBe("/remote/workspace/.bullpen-runtime/pi/sessions/session-123.jsonl");
  });

  it("starts a fresh remote Pi session when the saved session header is empty or unreadable", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-empty-header-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      context: {
        bullpenWorkspace: { cwd: workspaceDir, source: "project_primary" },
      },
      executionTransport: {
        remoteExecution: {
          host: "127.0.0.1",
  ...
  it("starts a fresh remote Pi session when the remote head command fails", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "bullpen-pi-remote-head-failure-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
