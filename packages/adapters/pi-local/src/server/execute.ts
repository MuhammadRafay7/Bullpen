...
const BULLPEN_SESSIONS_DIR = path.join(os.homedir(), ".pi", "bullpens");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[bullpen] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[bullpen] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[bullpen] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "bullpen-pi-skills-"));
  const target = path.join(tmp, "skills");
  await fs.mkdir(target, { recursive: true });
  const availableEntries = await readBullpenRuntimeSkillEntries(config, __moduleDir);
  const desiredNames = new Set(resolveBullpenDesiredSkillNames(config, availableEntries));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(BULLPEN_SESSIONS_DIR, { recursive: true });
  return BULLPEN_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(BULLPEN_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readBullpenRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveBullpenDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // Materialize custom Pi providers (BULLPEN_PI_PROVIDERS) into a managed
  ...
  try {
    ...
    // invoke skill binaries (e.g. `bullpen-get-issue`) by name. Without this,
    ...
    if (executionTargetIsRemote) {
      try {
        ...
        await onLog(
          "stdout",
          `[bullpen] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
        );
    ...
    if (runtimeSessionId && !canResumeSession) {
      const staleSessionCwdNote =
        savedSessionCwd !== null && !sessionHeaderCwdMatches
          ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so Bullpen will start a fresh session for "${effectiveExecutionCwd}".`
          : "";
      await onLog(
        "stdout",
        executionTargetIsRemote
          ? `[bullpen] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
          : `[bullpen] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
      );
    }
    ...
    if (resolvedInstructionsFilePath) {
      try {
        ...
        await onLog(
          "stdout",
          `[bullpen] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
        );
    ...
    const renderedBootstrapPrompt =
      !canResumeSession && bootstrapPromptTemplate.trim().length > 0
        ...
        : "";
    const wakePrompt = renderBullpenWakePrompt(context.bullpenWake, { resumedSession: canResumeSession });
    const shouldUseResumeDeltaPrompt = canResumeSession && wakePrompt.length > 0;
    const renderedHeartbeatPrompt = shouldUseResumeDeltaPrompt || isBullpenRecoveryWakePayload(context.bullpenWake)
      ? ""
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[bullpen] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
        );
