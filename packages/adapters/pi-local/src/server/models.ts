...
import type { AdapterModel } from "@bullpen/adapter-utils";
import { asString, runChildProcess } from "@bullpen/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.BULLPEN_PI_COMMAND === "string" &&
    process.env.BULLPEN_PI_COMMAND.trim().length > 0
      ? process.env.BULLPEN_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["BULLPEN_", "npm_", "NPM_"] as const;
