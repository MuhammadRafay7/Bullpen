...
test("renders standard assignment wake with task authority and no backlog discovery guidance", () => {
  ...
  expect(prompt).toContain("Bullpen task context:");
...
test("renders scoped planning wake authority before the Hermes default workflow", () => {
  const prompt = buildPrompt(baseContext(), {
    bullpenApiUrl: "http://127.0.0.1:3101/api",
  });
...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## Bullpen Resume Delta");
  expect(prompt).toContain("You are resuming an existing Bullpen session.");
...
test("renders safe Bullpen API examples from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    bullpenApiUrl: "http://bullpen.local/api",
  });

  expect(prompt).toContain("Use `$BULLPEN_API_URL`, `$BULLPEN_API_KEY`, and `$BULLPEN_RUN_ID`");
  expect(prompt).toContain("Displayed command logs may redact secrets");
  expect(prompt).toContain('-H "Authorization: Bearer $BULLPEN_API_KEY"');
  expect(prompt).toContain('-H "X-Bullpen-Run-Id: $BULLPEN_RUN_ID"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  const prompt = buildPrompt(baseContext(), {
    ...
    promptTemplate: [
      ...
      "agent={{agent.name}}",
      "api={{bullpenApiUrl}}",
      "keyEnv={{bullpenApiKeyEnv}}",
      "runEnv={{bullpenRunIdEnv}}",
      "wakePrompt={{bullpenWakePrompt}}",
      "task={{bullpenTaskMarkdown}}",
      "wakeJson={{bullpenWakeJson}}",
  ...
  expect(prompt).toContain("api=http://bullpen.local/api");
  expect(prompt).toContain("keyEnv=BULLPEN_API_KEY");
  expect(prompt).toContain("runEnv=BULLPEN_RUN_ID");
  expect(prompt).toContain("wakePrompt=## Bullpen Wake Payload");
  expect(prompt).toContain("task=Bullpen task context:");
  expect(prompt).toContain("wakeJson={\"reason\":\"issue_assigned\"");
  expect(prompt).toContain('"reason":"issue_assigned"');
  expect(prompt).toContain("## Bullpen Wake Payload");
  expect(prompt).toContain("Issue description:\n```text\nUse the wake payload as runtime authority.\n```");
  expect(prompt).not.toContain("Bullpen runtime identity:");
});
