...
/**
 * Parse Hermes Agent stdout into TranscriptEntry objects for the Bullpen UI.
 *
 ...
 *   System:     "[hermes] ..."
 *
 * We emit structured tool_call/tool_result pairs so Bullpen renders proper
 ...
 * Bullpen uses this to match them in normalizeTranscript.
 */
 ...
 * Bullpen renders proper tool cards with status icons and expand/collapse.
 *
...
export function parseHermesStdoutLine(
  ...
  if (trimmed.startsWith("[hermes]") || trimmed.startsWith("[bullpen]")) {
  ...
  // Emit as stderr so Bullpen groups them into the amber accordion.
