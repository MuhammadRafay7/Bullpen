 ...
 *
 * Translates Bullpen's CreateConfigValues into the adapterConfig
...
import type { CreateConfigValues } from "@bullpen/adapter-utils";
...
/**
 * Build a Hermes Agent adapter config from the Bullpen UI form values.
 */
export function buildHermesConfig(
  ...
  // NOTE: Provider is NOT set here because the Bullpen UI form
  ...
  // Execution limits — let the user configure these from the Bullpen UI.
  ...
  // Heartbeat config is handled by Bullpen itself
