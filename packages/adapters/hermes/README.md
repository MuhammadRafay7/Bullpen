# Bullpen Adapters for Hermes Agent

A [Bullpen](https://paperclip.ing) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a Bullpen company.
...
Choose `hermes_local` when Bullpen and Hermes run on the same trusted host
and Bullpen should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both Bullpen-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so Bullpen renders proper tool cards with status icons and expand/collapse
...
This package ships with Bullpen core as the built-in `hermes_local` and
...
manager, or add an entry to `~/.bullpen/adapter-plugins.json` and restart
Bullpen. The external package can override either built-in Hermes adapter
while it is enabled:

```json
[
  {
    "packageName": "@bullpen/hermes-bullpen-adapter",
    "localPath": "/absolute/path/to/bullpen/packages/adapters/hermes",
    "type": "hermes_local",
...
Bullpen core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in Bullpen

In the Bullpen UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. Bullpen injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in Bullpen
...
```json
{
  ...
  "adapterConfig": {
    ...
    "bullpenApiUrl": "http://127.0.0.1:3100",
...
`http://127.0.0.1:9119/chat`, Bullpen maps it to
`http://127.0.0.1:9119/api` before calling Hermes API routes. `/chat` and the
dashboard root are browser UI routes; Bullpen tests `/api/health` and starts
...
`@bullpen/adapter-hermes-gateway` remains as a deprecated compatibility shim
for one release. It re-exports the gateway entrypoints from
`@bullpen/hermes-bullpen-adapter/gateway` and preserves the legacy exports
for existing plugin installs. New installs and built-in Bullpen registrations
should use `@bullpen/hermes-bullpen-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives Bullpen runtime identity through environment variables:
...
`Authorization: Bearer $BULLPEN_API_KEY` on Bullpen API requests and
`X-Bullpen-Run-Id: $BULLPEN_RUN_ID` on mutating issue requests. For
multiline comments or status updates, preserve newlines with a heredoc plus
`jq --arg`.

### Hermes-originated Bullpen tasks

The package includes a Hermes skill/helper for the reverse direction: a user
starts in Hermes and asks Hermes to create or update Bullpen work. This is not
the same as Bullpen waking Hermes through `hermes_local` or `hermes_gateway`.

Configure Bullpen access in Hermes env/profile secrets, not prompt text:

```bash
BULLPEN_API_URL=http://127.0.0.1:3100/api
BULLPEN_BRIDGE_API_KEY=<task-bridge-scoped-agent-api-key>
```
...
The bundled `bullpen-task-bridge` skill provides deterministic helper
commands:

```bash
node ./bullpen-task.mjs list-assigned
node ./bullpen-task.mjs create-task --parent-id "<approved-parent-issue-id>" --title "Investigate checkout failures" --description "Capture failing request and root cause."
...
Create issues in Bullpen and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to Bullpen
...
| `bullpenApiUrl` | string | `http://127.0.0.1:3100/api` | Bullpen API base URL |
...
| `{{bullpenApiUrl}}` | Bullpen API base URL |
...
6. **Reports** results back to Bullpen with cost, usage, and session state
...
- **Bullpen-managed skills** — bundled with the adapter, togglable from the UI
...
Bullpen UI can display both managed and native skills in one view.

## Development

```bash
git clone https://github.com/paperclipai/paperclip
cd bullpen/packages/adapters/hermes
pnpm install
pnpm build
```
...
- [Bullpen](https://github.com/paperclipai/paperclip) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [Bullpen Docs](https://docs.paperclip.ing) — Bullpen documentation
