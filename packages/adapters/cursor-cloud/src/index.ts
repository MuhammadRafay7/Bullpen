...
Use when:
- You want Bullpen to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across Bullpen heartbeats
- You want Bullpen to keep task state while Cursor handles remote code execution
...
Notes:
- Bullpen reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each Bullpen heartbeat maps to a Cursor run on that durable agent.
- Bullpen injects BULLPEN_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- Bullpen remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
