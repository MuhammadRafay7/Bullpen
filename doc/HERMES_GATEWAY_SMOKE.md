...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or Bullpen. It
...
- Board/operator auth is required through `BULLPEN_AUTH_HEADER`,
  `BULLPEN_COOKIE`, or a board-capable `BULLPEN_API_KEY`.
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed Bullpen agent key and is written
  `chmod 600`.
...
- `BULLPEN_API_URL`: Bullpen URL used by the operator shell.
- `BULLPEN_API_URL_FOR_HERMES`: Bullpen URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the Bullpen
  adapter, reachable by the Bullpen server.
...
## Docker Desktop or Linux host Bullpen

Use this when Bullpen runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as Bullpen

Use this when Bullpen is a container on a Docker network and the Hermes smoke
...
BULLPEN_API_URL=http://127.0.0.1:3100 \
BULLPEN_AUTH_HEADER='Bearer <board-token>' \
HERMES_CONTAINER_NAME=bullpen-hermes-gateway-smoke \
HERMES_SMOKE_NETWORK=bullpen_default \
HERMES_DOCKER_ADD_HOST=0 \
HERMES_GATEWAY_API_BASE_URL=http://bullpen-hermes-gateway-smoke:8642 \
HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642 \
BULLPEN_API_URL_FOR_HERMES=http://bullpen:3100 \
HERMES_GATEWAY_ALLOW_INSECURE_HTTP=1 \
pnpm smoke:hermes-gateway-e2e
```

Change `bullpen_default` and `bullpen` to your Compose network and service
name. The unsafe HTTP flag is required because Bullpen stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network Bullpen

Use this when Bullpen is exposed on a private IP or tailnet address and the
Hermes container can reach that address.

```sh
BULLPEN_API_URL=http://192.168.1.20:3100 \
BULLPEN_AUTH_HEADER='Bearer <board-token>' \
BULLPEN_API_URL_FOR_HERMES=http://192.168.1.20:3100 \
...
Use this when Bullpen should talk to Hermes through a TLS hostname. The smoke
container still publishes a local port, and your reverse proxy forwards the TLS
hostname to that port.

```sh
BULLPEN_API_URL=https://bullpen.example.com \
BULLPEN_AUTH_HEADER='Bearer <board-token>' \
BULLPEN_API_URL_FOR_HERMES=https://bullpen.example.com \
...
`HERMES_GATEWAY_API_BASE_URL` to the URL Bullpen should store on the adapter.
