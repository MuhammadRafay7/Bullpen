...
Bullpen uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.bullpen/instances/default/db/` directory for storage
2. Ensures the `bullpen` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.bullpen/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Bullpen config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/bullpen` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://bullpen:bullpen@localhost:5432/bullpen
```
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Bullpen uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for Bullpen until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.bullpen/instances/default/db/`) |
...
Bullpen stores current-user sidebar membership state in:
...
Bullpen supports automatic and manual logical database backups. These dumps include
...
`bullpen db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
Bullpen stores secret metadata and versions in:
...
- Default key file: `~/.bullpen/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.bullpen/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `BULLPEN_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
