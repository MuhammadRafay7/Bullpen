...
Use this guide when a Hermes runtime should join Bullpen as an external
...
Bullpen ships both Hermes adapters as built-ins:
...
`@bullpen/hermes-bullpen-adapter` package to override or shadow a built-in
adapter while developing the Hermes package. If the external override is paused
or removed, Bullpen restores the built-in `hermes_local` / `hermes_gateway`
adapter.
...
- Hermes gateway key: set `API_SERVER_KEY` before starting Hermes. Bullpen
  stores the same value as `agentDefaultsPayload.apiKey` so it can call Hermes.
- Bullpen agent key: created after the board approves the join request and
  claimed once by the Hermes agent. Hermes uses this key as
  `BULLPEN_API_KEY` when it calls Bullpen.

Do not reuse the Hermes gateway key as the Bullpen agent key. The Hermes
gateway key authenticates Bullpen-to-Hermes traffic; the claimed Bullpen key
authenticates Hermes-to-Bullpen traffic.
...
Bullpen can usually store `http://127.0.0.1:8642` as the gateway URL. For
Docker, LAN, tailnet, or reverse-proxy setups, use a URL reachable by the
Bullpen server process.
...
- `GET /api/skills/bullpen`

For CLI-driven setup, create and inspect the invite directly:

```sh
pnpm bullpen invite create --company-id <company-id> --payload-json '{"requestType":"agent"}'
...
- `agentDefaultsPayload.apiBaseUrl` is the Hermes gateway URL that Bullpen
  calls.
- `agentDefaultsPayload.bullpenApiUrl` is the Bullpen base URL that Hermes
  can call after approval and key claim.
- `BULLPEN_API_URL` / `BULLPEN_API_KEY` are injected runtime values for
  Hermes-originated Bullpen API calls after the agent is approved.

## Approve And Claim

After Hermes submits the join request:

1. In Bullpen, review the pending agent join request.
...
4. Store the claimed Bullpen key in Hermes runtime state or secrets. The claim
...
agent and wake it through the normal Bullpen heartbeat path.

## Local Fresh-State Smoke

For a fresh Docker-backed Hermes gateway and end-to-end Bullpen join/run
verification, use:
...
- creates and approves a Bullpen agent-only invite
- joins as `hermes_gateway`
- wakes the agent on a smoke issue
- removes Bullpen and Docker test state on success
...
- CLI invite flow: `pnpm bullpen invite create`, `invite show`,
...
- Adapter development override: Adapter manager can install
  `@bullpen/hermes-bullpen-adapter` as an external override, but normal
