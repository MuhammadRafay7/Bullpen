...
This spec defines how Bullpen actually runs agents while staying runtime-agnostic.
...
1. Bullpen is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Bullpen manages lifecycle, and on exit Bullpen parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `bullpen.skill` (shared Bullpen skill text block)
...
2. Preferred transport is env vars (`BULLPEN_*`) injected at runtime.
