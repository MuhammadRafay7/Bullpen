...
For `claude_local` and `codex_local`, Bullpen assumes the CLI is already installed and authenticated on the host machine.
...
Bullpen stores resumable session state per `(agent, taskKey, adapterType)`.
