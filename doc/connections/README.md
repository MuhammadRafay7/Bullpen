...
Post-read action: classify a new integration request, pick the right Bullpen
...
- **D5: inbound stays thin.** External clients that call Bullpen use scoped
  Bullpen tokens and existing profiles/rules. They do not get a separate
  permission model.
...
Bullpen run/session token; the server or broker resolves the connection,
...
| --- | --- | --- | --- |
| **P1. Sign-in methods** | *Who are you?* | `bullpen-id` (id.paperclip.ing → Account) | Minimal-scope provider tokens (`openid email profile`), used once to authenticate, encrypted at rest, never exported |
| **P2. Connections (Apps)** | *What may your agents touch?* | Bullpen App instances (`tool_connections`), acquired via the **connect broker** for hosted + self-hosted | Rich-scope, long-lived resource tokens in the **instance's** encrypted vault; per-agent grants; ask-first on writes |
| **P3. Login with Bullpen** | *Who may authenticate against us?* | `bullpen-id` OIDC provider + DB-backed client registry | Our ES256 ID/access tokens issued *by* us to registered RPs (instances, the broker, future third parties) |
...
| --- | --- | --- |
| Bullpen App instances | P2 | **"Connections"** |
