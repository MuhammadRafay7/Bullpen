...
2. **Connection operations are company-scoped and brokered by Bullpen.**
...
- Bullpen objects mutated by integrations: issues, comments, documents,
...
- Route webhook payloads only to the owning company/connection. Ignore request
  ...
  connection.
- Apply resource filters before storing external mappings or mutating Bullpen.
- For revoked/disabled connections, acknowledge where provider semantics require
  it, log the filtered/drop outcome, and do not mutate Bullpen state.
...
- Provider OAuth/app-installation scopes may be broader than Bullpen resource
  filters. Bullpen must enforce the narrower internal filter.
