...
| Subject | The app/workspace or Bullpen user on whose behalf a credential is requested. | The calling agent. |
...
| Trigger | Provider-origin event definition that starts governed Bullpen work. | An unauthenticated webhook handler. |
| Connector service | Bullpen-operated relay for managed OAuth callbacks, credential custody, and webhook intake at `connect.paperclip.ing`. | Bullpen ID or the per-company broker. |
...
| Skill | Agent instruction package that calls governed connections through Bullpen. |
...
- Inbound clients use scoped Bullpen auth; outbound vendor calls use the Apps
