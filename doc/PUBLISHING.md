...
Low-level reference for how Bullpen packages are prepared and published to npm.
...
Bullpen no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `bullpen`, imports code from workspace packages such as:
...
### Manual first publish for `@bullpen/ui`
...
npm view @bullpen/ui version

# make sure the dist payload is fresh
pnpm --filter @bullpen/ui build
...
- If `npm view @bullpen/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@bullpen/ui`, the `bullpen` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Bullpen uses calendar versions:
...
- every published internal `@bullpen/*` dependency referenced by that manifest exists on npm
...
Bullpen does not auto-publish every non-private workspace package anymore.
...
```bash
# safe preview
pnpm run release:bootstrap-package -- @bullpen/new-package

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @bullpen/new-package --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@bullpen/new-package`
2. go to `Settings` → `Trusted publishing`
3. add repository `bullpen/bullpen`
