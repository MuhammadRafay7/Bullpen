...
Bullpen ships core trust preset names so containment decisions are enforced in
...
scope for risky access, Bullpen fails closed.
...
agent can read or mutate through the Bullpen API and prevents raw untrusted
...
Managed `low_trust_review` runs fail closed unless Bullpen can enforce the
runtime boundary:
...
local review, but Bullpen-managed low-trust execution requires a sandboxed
