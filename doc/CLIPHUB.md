...
ClipHub is the public registry where people share, discover, and download Bullpen company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Bullpen what a package registry is to a programming language. Bullpen already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Bullpen — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Bullpen instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Bullpen spec. It contains:
...
**Install (fresh start):**
```
bullpen install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Bullpen instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Bullpen, export your company as a template, then publish:

```
bullpen export --template my-company
bullpen publish cliphub my-company
```
...
ClipHub is a **separate service** from Bullpen itself. Bullpen is self-hosted; ClipHub is a hosted registry that Bullpen instances talk to.
...
| **Bullpen CLI** | `bullpen install`, `bullpen publish`, `bullpen cliphub sync` — built into Bullpen |
| **Bullpen UI** | "Browse ClipHub" panel in the Bullpen web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Bullpen) |
| Backend | TypeScript + Hono (consistent with Bullpen) |
| Database | PostgreSQL |
...
4. Run `bullpen install cliphub:acme/lean-saas-shop`
5. Bullpen creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Bullpen until it works well
2. Export: `bullpen export --template my-agency`
3. Publish: `bullpen publish cliphub my-agency`
...
3. Install just that agent: `bullpen install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Bullpen. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Bullpen what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `bullpen install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Bullpen web UI
- [ ] `bullpen cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Bullpen's own philosophy)
