...
Bullpen supports a managed installation, an ephemeral `npx` tryout, a
...
3. delegates installation to `bullpen install`;
...
```sh
raw_base=https://raw.githubusercontent.com/bullpen/bullpen
...
Each installer flag also has a `BULLPEN_INSTALL_*` environment-variable
...
The `bullpen` shim remains stable while `current` switches atomically
between complete payloads. Bullpen keeps the two previous managed payloads
for rollback. Configuration, databases, uploads, logs, secrets, and workspaces
remain under `~/.bullpen/instances/` and are not stored inside CLI payloads.
...
```sh
npx --registry https://registry.npmjs.org bullpen install
```

Install canary or pin an exact published version:

```sh
npx --registry https://registry.npmjs.org bullpen install --canary
npx --registry https://registry.npmjs.org bullpen install --version 2026.720.0
```

Install a branch, tag, or commit from GitHub:

```sh
npx --registry https://registry.npmjs.org bullpen install --ref master
npx --registry https://registry.npmjs.org bullpen install --ref v2026.720.0
npx --registry https://registry.npmjs.org bullpen install --ref <commit-sha>
```
...
Interactive onboarding asks whether Bullpen should run as a background
...
```sh
bullpen onboard --yes                    # configure only; no service install
bullpen onboard --yes --install-service  # explicit automation opt-in
...
```sh
bullpen service install
bullpen service status
bullpen service start
bullpen service stop
bullpen service restart
bullpen service logs -f
bullpen service uninstall
```

Bullpen uses a systemd user service on Linux and WSL2 systems with user
systemd, and a LaunchAgent on macOS. Containers, WSL1, and systems without a
supported user service manager receive foreground `bullpen run` guidance
...
Use one server process per instance. `bullpen run` refuses to start when
...
```sh
bullpen update --latest
bullpen update --canary
bullpen update --version 2026.720.0
```
...
If the service is stopped, start it with `bullpen service start` before
updating so Bullpen can take the safety backup. Use
`bullpen update --no-backup` only when you intentionally accept updating
...
```sh
npm install --global --registry https://registry.npmjs.org bullpen
bullpen onboard
```
...
The managed `bullpen update` command can update managed and global npm
...
`bullpen uninstall` removes the managed shim, manifest, and CLI payloads.
It deliberately preserves `~/.bullpen/instances/`, including configuration,
databases, uploads, logs, secrets, backups, and workspaces. Back up and remove
that data separately only when you intend to delete the Bullpen instance.
