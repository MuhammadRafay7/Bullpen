# Bullpen Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Bullpen's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Bullpen app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Bullpen HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@bullpen/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Bullpen plugin design is based on the following assumptions:

1. Bullpen is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Bullpen business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Bullpen core.
...
2. Let plugins add major capabilities without editing Bullpen core.
...
4. Depend on project-local plugin folders such as `.bullpen/plugins`.
...
The single Bullpen deployment an operator installs and controls.

### 5.2 Company

A first-class Bullpen business object inside the instance.
...
A trusted in-process extension loaded directly by Bullpen core.
...
An installable instance-wide extension package loaded through the Bullpen plugin runtime.
...
Bullpen has two extension classes.
...
Bullpen already has a concrete workspace model:
...
Plugins live under the Bullpen instance directory.

Suggested layout:

- `~/.bullpen/instances/default/plugins/package.json`
- `~/.bullpen/instances/default/plugins/node_modules/`
- `~/.bullpen/instances/default/plugins/.cache/`
- `~/.bullpen/instances/default/data/plugins/<plugin-id>/`
...
Bullpen should add CLI commands:

- `pnpm bullpen plugin list`
- `pnpm bullpen plugin install <package[@version]>`
- `pnpm bullpen plugin uninstall <plugin-id>`
- `pnpm bullpen plugin upgrade <plugin-id> [version]`
- `pnpm bullpen plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@bullpen/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface BullpenPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
- `minimumHostVersion` is preferred, with `minimumBullpenVersion` retained for
...
Plugins may contribute tools that Bullpen agents can use during runs.
...
- Bullpen server starts one worker process per installed plugin
...
Receives one typed Bullpen domain event.
...
Trusted orchestration plugins can create and update Bullpen issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Bullpen resources rather than using private plugin state:
...
6. For recurring business workflows that should create visible Bullpen work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@bullpen/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@bullpen/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@bullpen/plugin-sdk/ui`)
...
Bullpen-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Bullpen navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@bullpen/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@bullpen/plugin-sdk/ui`
...
The `@bullpen/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that stores the shared `{ type: "secret_ref", secretId, version? }` object shape and resolves through the Bullpen secret provider system rather than a plain text input
...
1. Core Bullpen data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Bullpen objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Bullpen product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Bullpen later decides git state is core product data, it should become a first-party table too
...
3. Secret refs resolve through the existing Bullpen secret provider system using explicit `companyId`.
...
6. The operator may force-purge immediately via CLI: `pnpm bullpen plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Bullpen server. This is a normative requirement, not optional.
...
### 27.1 `@bullpen/plugin-test-harness`
...
```ts
import { createTestHarness } from "@bullpen/plugin-test-harness";
...
For developing a plugin against a running Bullpen instance:

- The operator installs the plugin from a local path: `pnpm bullpen plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-bullpen-plugin`) that scaffolds:

- `package.json` with correct `bullpenPlugin` keys
...
- `@bullpen/plugin-workspace-files`
- `@bullpen/plugin-terminal`
- `@bullpen/plugin-git`
- `@bullpen/plugin-linear`
- `@bullpen/plugin-github-issues`
- `@bullpen/plugin-grafana`
- `@bullpen/plugin-runtime-processes`
- `@bullpen/plugin-stripe`
...
- `@bullpen/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@bullpen/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@bullpen/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@bullpen/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@bullpen/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@bullpen/plugin-sdk/ui`
...
- `@bullpen/plugin-test-harness`
- `create-bullpen-plugin` starter template
...
Bullpen should not implement a generic in-process hook bag modeled directly after local coding tools.

Bullpen should implement:
...
That is the complete target design for the Bullpen plugin system.
