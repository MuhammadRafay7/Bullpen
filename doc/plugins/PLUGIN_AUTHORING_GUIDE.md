...
This guide describes the current, implemented way to create a Bullpen plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Bullpen app.
...
- The host provides a small shared React component kit through
  `@bullpen/plugin-sdk/ui`; use it for common Bullpen controls before
...
Bullpen owns inline markdown rendering; plugins must not return React, HTML,
...
```bash
bullpen plugin init @yourscope/plugin-name --output /absolute/path/to/plugin-repos
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@bullpen/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@bullpen/plugin-sdk` from the local Bullpen checkout into a `.bullpen-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/bullpen/packages/plugins/sdk` if you have more than one Bullpen checkout.

## Local development workflow

See the short [Local Plugin Development guide](./LOCAL_PLUGIN_DEVELOPMENT.md) for the full happy path (`pnpm dev` → `bullpen plugin install <absolute-path>` → `bullpen plugin list`) and reload semantics.
...
## Managed Bullpen resources

Plugins that provide durable Bullpen business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Bullpen agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Bullpen issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Bullpen managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Bullpen heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Bullpen issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Bullpen records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@bullpen/plugin-sdk/ui`
...
Use shared components from `@bullpen/plugin-sdk/ui` when the plugin needs a
Bullpen-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Bullpen UX patterns to reduce drift and
...
Use `FileTree` from `@bullpen/plugin-sdk/ui` whenever the plugin only needs
